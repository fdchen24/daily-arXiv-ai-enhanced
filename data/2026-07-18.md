...
TL;DR: SceneBind提出了一种全模态场景表示方法，通过结合全局语义嵌入与以对象为中心的语义-空间槽，实现了视觉、音频和语言模态下联合的语义与3D空间理解，在场景检索和空间定位任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有全模态编码器擅长实例级语义理解（场景中有什么），但缺乏显式的空间结构信息（物体在哪里），无法有效支持跨模态场景检索和物体定位等需要空间理解的任务。

Method: 将每个场景表示为语义-空间实体：全局语义嵌入 + 以对象为中心的语义-空间槽（显式捕获对象级语义、空间属性和不确定性）；提出SceneBind Matching方案，整合全局场景相似度与对象对齐；构建真实世界双耳音视频数据集并设计跨模态对齐训练协议。

Result: 在场景检索和空间检索任务上达到SOTA；支持强零样本迁移到音视频定位等下游任务；与大规模预训练语义编码器兼容，仅需少量额外token即可实现轻量级空间建模。

Conclusion: SceneBind通过语义-空间联合表示有效弥补了现有全模态模型缺乏空间结构的不足，为跨模态场景理解与定位提供了统一的框架。
...
TL;DR: 本文提出MultiRef-Compass，首个针对多参考到音视频（MR2AV）生成的统一基准，包含350个样本、14项子指标的四维评估体系，实验表明现有系统在多个维度仍有显著改进空间。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注文本驱动生成、单参考主体保持或孤立的音视频对齐，缺乏对MR2AV（要求模型联合推理多个参考并生成同步视觉与音频内容）的全面评估。

Method: 通过可扩展可控的资产组合管线构建350个精心筛选的样本，覆盖多视角主体保持、多实体绑定、人-物-场景组合；定义包含基本质量、参考一致性、音视频一致性、指令遵循四个维度的14项子指标评估协议；集成自动指标与重判增强的MLLM-as-a-Judge框架。

Result: 在八个代表性MR2AV系统上的广泛实验显示，这些系统在多个评估维度上仍有显著改进空间。

Conclusion: MultiRef-Compass作为MR2AV研究的基础基准，填补了该领域评估空白，为未来研究奠定基础。
...
TL;DR: SIRUS是一种无需训练的推理时文本到视频（T2V）概念遗忘框架，通过定位目标概念相关的提示证据并在采样过程中抑制其表达，在不更新网络参数的情况下实现概念移除。在CogVideoX和Wan2.2上，SIRUS在遗忘成功率、帧命中率和视频质量保持上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: T2V生成器能合成逼真且时序一致的视频，但如何可控地移除生成器中的目标概念仍然困难。与文本到图像的概念擦除不同，T2V遗忘需要在抑制跨帧持续存在的目标概念的同时，保留非目标主体、动作、场景和时序结构。

Method: SIRUS是一种无需训练的推理时框架。给定目标概念的文本别名，它在采样过程中定位与目标相关的提示证据并抑制目标表达，无需更新文本编码器或去噪网络。同时提出了面向视频的T2V遗忘评估框架，分别衡量目标遗忘、非目标保留、视频质量、越狱鲁棒性和效率。

Result: 在CogVideoX上，SIRUS达到70.4%的平均遗忘成功率和25.7%的平均帧命中率，优于VideoEraser的44.4%/47.2%；同时将平均VBench质量下降从-0.043降至-0.016，实现了最强的遗忘-质量权衡。在Wan2.2上的迁移实验表明SIRUS可泛化至现代T2V骨干网络。

Conclusion: SIRUS在T2V概念遗忘任务上有效且高效，无需重新训练，在遗忘效果和生成质量之间取得了更好的平衡，并具有良好的跨模型泛化能力。
...
TL;DR: KeyFrame-Compass 是首个用于评估关键帧条件视频生成的综合基准，包含386个精心策划的样本，覆盖多种应用场景、视频结构、提示粒度、条件格式和关键帧密度。实验发现当前模型在忠实执行关键帧与自然视频合成之间存在明显权衡，且性能随关键帧密度增加而下降。


<details>
  <summary>Details</summary>
Motivation: 尽管现有模型支持多关键帧条件，但仍不清楚它们能否在保持整体视频质量的同时忠实地再现指定的关键帧。缺乏统一的评估基准来衡量关键帧条件视频生成的效果。

Method: 构建包含386个样本的基准，涵盖3个应用领域、2种视频结构、2种提示粒度、2种条件格式和4种关键帧密度。提出自动化评估框架，将关键帧执行分解为6个互补指标（存在性、保真度、时序顺序、定位、持久性、唯一性），并通过基于证据的MLLM判断和专用感知模型评估整体视频质量。

Result: 在9个代表性视频生成系统上的实验揭示了当前模型的根本性局限：忠实执行关键帧与自然视频合成之间存在明显权衡；性能随着关键帧约束密度增加而下降；大多数开源模型无法将故事板网格输入正确解释为时序有序的关键帧序列。

Conclusion: 当前关键帧条件视频生成模型在关键帧执行和视频质量之间难以兼顾，密度越高的约束越容易导致性能退化。该基准为未来研究提供了系统性的评估框架。
...
TL;DR: 本文探究视觉基础模型的潜在特征空间是否内在地编码了3D欧几里得空间的结构属性，并提出新的探针方法从拓扑和几何两个角度验证这一关系，进而提出潜空间导航技术实现纯潜空间的视觉里程计与定位。


<details>
  <summary>Details</summary>
Motivation: 现有工作通过回归深度、法线等图像中心量来探测视觉特征的3D感知能力，但缺乏对特征空间结构与3D欧几里得变换群SE(3)之间关系的系统性研究。本文旨在揭示自监督视觉模型是否隐式地学习到3D空间的内在结构。

Method: 提出两类探针：(1) 互邻域度量，用于衡量特征空间中的邻域结构与空间拓扑的对齐程度；(2) Poincaré适配器，用于测试从静态场景的潜位移中线性恢复相机运动几何的可行性。基于这些发现，提出潜空间导航技术，在潜空间中直接进行视觉里程计与定位。

Result: 自监督视觉模型虽然未经过显式的3D监督或主动代理训练，但其潜在子空间与三维欧几里得空间存在显著强相关。潜空间导航技术能够实现纯潜空间的视觉里程计与定位，无需显式3D重建。

Conclusion: 视觉基础模型的特征空间内在地编码了3D欧几里得空间的结构属性，这一发现为无需显式3D重建的视觉导航任务提供了新的技术路线。
...
TL;DR: 本文提出了一种面向赛车场景的3D车道检测新方法，通过引入包含25万+图像和惯性测量数据的新数据集，结合多摄像头集成与惯性预积分技术，在保持近300Hz处理速度的同时将F1分数提升至0.9以上，横向MAE降至0.18m以下。


<details>
  <summary>Details</summary>
Motivation: 现有车道检测研究主要针对常规驾驶场景，缺乏对高速、极端道路几何形状的赛车场景的探索。作者旨在填补这一空白，研究赛车场景下的3D车道检测问题。

Method: 1) 构建包含25万+图像、多摄像头及惯性测量数据的新数据集；2) 提出多摄像头集成方法；3) 利用惯性测量数据进行预积分，跨摄像头和时间回归道路几何；4) 在硬件上验证集成方法。

Result: 1) 处理速度接近300Hz；2) 相比BevLaneDet等方法，F1分数提升3个百分点；3) 近车平均绝对误差(MAE)降低超过30%；4) 实际部署中F1>0.9，横向MAE<0.18m。

Conclusion: 惯性测量数据和多摄像头集成对于提升赛车场景下车道检测性能有效，该方法在高速、极端几何条件下仍能保持高精度和高帧率。
...
TL;DR: MonteRET是一个区域感知的检索增强框架，用于生成胸部CT报告。它整合全局和区域级CT特征，通过预测的医学条件和视觉-语言对齐检索临床相关知识，并利用知识引导的报告重写智能体优化报告。在RadGenome-ChestCT（24,128训练/1,564测试）和外部82例CT上评估，优于基线及SOTA方法，尤其在召回率方面提升显著，放射科医生评估也偏好MonteRET。


<details>
  <summary>Details</summary>
Motivation: 自动胸部CT报告生成面临挑战：临床可靠的报告既需要全容积理解，又需要准确描述局部解剖发现。现有方法难以同时满足这两个要求。

Method: MonteRET框架包含三个核心组件：1）整合全局CT特征与区域级解剖表示；2）利用预测的医学条件和区域级视觉-语言对齐检索临床相关知识；3）通过知识引导的报告重写智能体优化初始报告。

Result: 在RadGenome-ChestCT测试集（1,564 CT）和外部验证集（82 CT，来自纽约长老会/威尔康奈尔医学中心）上，MonteRET在报告质量、语义相似度和临床效能方面均优于匹配基线和多个SOTA方法。召回率提升最为显著，表明遗漏发现更少。放射科住院医师的人工专家评估也倾向于MonteRET。

Conclusion: MonteRET通过区域感知和检索增强策略有效提升了胸部CT报告生成的临床忠实度，尤其减少了发现遗漏问题，为自动化放射报告生成提供了有前景的方向。
...
TL;DR: 提出XCT-SAM框架，通过顺序参数高效微调策略（先在合金微结构数据集上微调Conv-LoRA适配器，再迁移到XCT图像），解决了增材制造XCT缺陷分割中类别不平衡和域迁移问题，在仅训练约415万参数（冻结99%以上模型参数）的情况下，在OOD基准和真实NIST XCT扫描上均优于零样本SAM和其他域适应基线。


<details>
  <summary>Details</summary>
Motivation: 增材制造XCT图像缺陷分割面临严重的类别不平衡和不同扫描条件间的大分布偏移问题。虽然SAM等基础模型提供了强大的通用分割先验，但其在自然图像上的预训练难以迁移到AM XCT领域（缺陷表现为微妙的非语义微观结构异常），且适应过程受到大域差距和标注真实XCT数据稀缺的限制。

Method: 提出XCT-SAM顺序参数高效适应框架：先在全金微结构数据集上微调Conv-LoRA适配器（注入卷积空间归纳偏置到SAM骨干网络），再将适配后的模型迁移到XCT图像，逐步缩小域差距。使用秩r=2的Conv-LoRA适配器，仅训练约415万参数（冻结99%以上模型参数）。

Result: 在OOD CycleGAN-XCT基准和真实NIST XCT扫描上，XCT-SAM均一致优于零样本SAM和其他域适应SAM基线，取得了最佳的整体IoU和Dice分数。

Conclusion: 证明了中间域适应结合参数高效适配器策略对于工业XCT缺陷分割的有效性。
...
TL;DR: 本文提出DCVC-Mamba（DCVC-MB），一种用于B帧编码的神经视频编解码框架，采用IBP帧策略、基于状态空间模型的时空融合模型、熵感知跳过机制以及两种推理时策略，在BD-rate指标上相比现有神经视频编解码器平均降低8.98%，相比VTM-19.0-LDP和VTM-19.0-RA(Inter-GoP=16)分别提升30.45%和1.81%。


<details>
  <summary>Details</summary>
Motivation: 现有神经视频编码（NVC）在B帧编码方面性能有限，缺乏高效的时空融合机制和低延迟编码策略，且熵编码计算开销大，需要一种更高效的B帧编码框架。

Method: 1) 采用IBP帧策略实现低延迟B帧编码；2) 提出基于状态空间模型（SSM）的时空融合模型用于双向时序预测；3) 设计熵感知跳过机制，选择性省略部分潜变量的编码以降低熵编码时间；4) 引入两种推理时策略以进一步提升压缩性能。

Result: BD-rate相比现有NVC平均降低8.98%；相比VTM-19.0-LDP降低30.45%；相比VTM-19.0-RA(Inter-GoP=16)降低1.81%。

Conclusion: DCVC-MB在B帧编码任务上显著超越现有NVC和传统编码标准（如VTM），验证了状态空间模型在视频压缩中的有效性，推动了神经视频编码技术的发展。
...
TL;DR: 本文提出SD-MAR框架，通过合成数据训练VLM进行多图像分析推理，结合GRPO-lite与BDA的强化学习策略，显著提升模型在分析推理任务上的表现（最高提升36.95%），同时保持甚至改善跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在多图像比较、变化检测和多步视觉推理等需要分析推理的任务上能力有限，而现有基准测试很少同时要求显式视觉比较和分析推理，导致这一能力未被充分探索。

Method: 1) 构建SD-MAR框架，通过受控扰动生成配对的视觉场景和推理任务（语义变化归因、定量比较）；2) 提出GRPO-lite结合BDA的强化学习方法，去除KL正则化以增强策略优化，并向后分配更多奖励给形成分析结论的后期推理步骤。

Result: 在Qwen2.5-VL-7B和InternVL3-8B上，SD-MAR域内准确率最高提升36.95%；Qwen2.5-VL-7B在SD-MAR基准上超越GPT-4.1；跨域泛化保持或提升（MME/MMMU-Pro/MathVista波动<1%，MMBench提升达4%）；LLM评判显示逻辑连贯性和解释质量持续改善。

Conclusion: SD-MAR框架有效弥补了VLM在多图像分析推理能力上的空白，GRPO-lite与BDA结合的方法既能提升域内性能，又能保持甚至增强泛化能力，为VLM在需要系统性视觉比较和推理的实际多模态应用中提供重要支撑。
...
TL;DR: MixCompress提出基于稀疏结构特化的统一可变码率图像压缩框架，通过混合专家（MoE）缓解梯度冲突，引入混合深度（MoD）动态扩展模型容量，结合条件辅助变换（CAT）进行子带能量调制，在计算高效的前提下超越单率率基线，建立新的帕累托前沿。
...
Method: 提出MixCompress统一框架：(1) 稀疏门控MoE路由缓解梯度冲突；(2) 引入MoD动态扩展模型容量以应对高码率表征需求；(3) 条件辅助变换（CAT）实现动态子带能量调制，实现层次化容量动态缩放。

Result: MixCompress不仅匹配甚至超越独立优化的单码率基线，在计算高效图像编码中建立了新的帕累托前沿。

Conclusion: 通过稀疏结构特化与容量动态缩放机制，有效解决可变码率图像压缩中的特征纠缠问题，在不增加计算预算的前提下实现更优的率失真性能。
...
TL;DR: 本文提出了一种针对区域损失函数（如Dice损失）导致的模型过度置信问题的梯度层面解决方案，通过在梯度向量场上进行"手术"——在损失偏导数中添加一个与预测误差线性相关的缩放因子——来有效缓解校准问题，在保持高预测精度的同时改善了模型校准性能。


<details>
  <summary>Details</summary>
Motivation: 基于区域的损失函数（如Dice损失）在高度类别和区域不平衡的分割任务中是事实标准，但这类损失函数训练出的模型普遍存在校准不良问题，倾向于产生过度置信的预测。在医学影像应用（如定义肿瘤切除边缘）中，这种校准问题阻碍了临床采用。

Method: 提出了一种对梯度向量场的"手术"干预方法：在损失函数的偏导数中添加一个因子，该因子使梯度幅度与预测误差呈线性关系缩放，从而缓解校准问题。

Result: 在2D和3D医学分割任务上的实证评估表明，该方法在保持高预测精度的同时，有效改善了模型校准问题，且可与任何基于区域的损失函数配合使用。

Conclusion: 梯度层面的简单干预即可有效缓解区域损失函数导致的过度置信问题，为医学影像分割的临床采用提供了更好的校准保障。
...
TL;DR: 提出动态操作超图框架，通过高阶关系建模和多实体编码，在细粒度操作识别任务上显著优于传统成对图方法和静态超图方法。


<details>
  <summary>Details</summary>
Motivation: 传统基于图的方法使用成对边，会将协调的事件分解为不连续的二元关系，难以捕捉细粒度操作中手、物体、工具及支撑面之间多实体协同演化的复杂关系。

Method: 构建动态操作超图：每时间步对多实体编码外观、空间、运动及语义角色特征；基于邻近性、接触和运动耦合谓词实例化并排序超边候选；通过节点-超边双向消息传递及时间注意力进行推理；输出类别无关的超边重要性分数。
...
Conclusion: 时变的高阶关系建模对于细粒度操作活动识别具有重要价值，超图框架能有效捕捉多实体间的复杂交互结构，优于传统成对图和静态建模方法。
...
TL;DR: K-NeAS提出了一种可扩展的多材料表面重建架构，通过共享隐层骨干网络、K材料顺序软选择器、GMM自动衰减边界和辅助浮点损失，在稀疏视角CT重建中实现了优于单材料基线1.88 dB的3D PSNR提升，并支持任意数量的组织材料分割。


<details>
  <summary>Details</summary>
Motivation: CT扫描存在辐射风险，稀疏视角重建至关重要。现有隐式场景表示方法虽能联合建模表面几何与衰减，但受限于手动调整衰减边界和刚性双材料约束，无法处理多组织重叠区域。

Method: 1. 用共享隐层骨干替代独立材料网络；2. 提出全可微的K材料顺序软选择器建模任意数量重叠组织；3. 用GMM自动确定衰减边界；4. 引入调度辅助浮点损失抑制稀疏视角下的几何幻觉。

Result: 在四个临床CBCT数据集上，K=3时腹部区域3D PSNR达33.28 dB（对比单材料NeAS基线31.40 dB，提升1.88 dB）；在5视角和10视角极端稀疏条件下，3D PSNR提升高达1.17 dB。

Conclusion: K-NeAS成功扩展到任意材料数量，在复杂多组织区域的体积保真度和稀疏采样鲁棒性上均显著优于基线方法，实现了自动化的多材料表面重建。
...
TL;DR: 本研究证明，无需面部特定训练，冻结的DINOv3自监督视觉模型的特征空间即可建立区域级人脸坐标系统，在跨身份匹配中达到83.0%语义准确率（随机基线23.0%），时间追踪准确率95.5%，且中间层（block 18）表现出最强的密集对应能力。


<details>
  <summary>Details</summary>
Motivation: 探究冻结的自监督视觉模型（DINOv3）是否无需人脸特定训练即可在面部区域（眼、眉、鼻、嘴、皮肤、毛发等）建立跨身份、跨时间的密集对应关系。

Method: 使用冻结的DINOv3 ViT-L/16 patch嵌入，仅以FaRL作为面部区域标签标注接口，在CelebDF-v2 200个真实视频上评估跨身份最近邻匹配和时序标签传播。通过有无FaRL控制实验及不同层对比分析。

Result: DINOv3在跨身份匹配中达83.0%区域语义准确率（随机基线23.0%），时序追踪准确率95.5%。无FaRL控制降至0.9%。中间层block 18的同类/跨类判别比达4.93倍（最终层仅1.48倍）。相比CLIP ViT-L/14，解剖区域上DINOv3高出16.8个百分点。

Conclusion: 冻结的DINOv3可作为强零样本区域级面部对应表征，中间层自监督特征最适用于密集人脸分析，而图像级对比学习（如CLIP）仅能捕获粗粒度布局。
...
TL;DR: 提出Cotton-SF YOLO框架，结合动态蛇形卷积和频域特征调制模块，在复杂田间环境下实现棉花幼铃的精准检测，mAP₅₀达0.8196。


<details>
  <summary>Details</summary>
Motivation: 棉花幼铃是棉花早期生殖生长的重要表型指标，但田间环境下存在小目标、遮挡、模糊、光照变化及与叶片对比度低等问题，现有方法对此探索不足。

Method: 基于YOLO26m，引入动态蛇形卷积（DySnakeConv）自适应提取变形边缘特征；设计频域特征调制模块，在C2f结构中融入频谱增强，重新校准频域表示以增强边缘和纹理信息，抑制叶片背景干扰。

Result: 在自建数据集上，mAP₅₀、mAP₅₀:₉₅和召回率分别为0.8196、0.4942和0.7939，较基线YOLO26m提升1.25%、3.45%和2.96%。

Conclusion: 结构特征与频域特征具有互补效应，二者结合可实现最佳检测性能，有效应对田间复杂环境下的棉花幼铃检测难题。
...
TL;DR: G2SR提出了一种高效的少视图表面重建方法，通过轻量级神经网络前端检测和跟踪2D高斯点，再通过解析后端三角化为3D高斯点，在保持高几何精度的同时实现了69-89 FPS的重建速度和极小的内存占用（203MB），比现有端到端方法快5-107倍。


<details>
  <summary>Details</summary>
Motivation: 现有的3D高斯点云重建方法在少视图条件下存在歧义性问题，易产生"浮点"伪影；端到端方法虽然能解决歧义性，但依赖大型Transformer网络，计算和内存开销大且泛化能力差。需要一种轻量、快速、几何准确且内存高效的方法。

Method: G2SR将任务分解为良态核心问题：利用跨视图的2D点对应关系，通过多视图几何分析求解3D点。具体采用轻量级神经前端在图像平面上检测和跟踪2D高斯点，再用解析后端三角化生成度量尺度的3D高斯点。

Result: 在ScanNet、Replica和DTU数据集上，G2SR在几何精度上达到或超过了最先进的端到端方法；在384×512分辨率下，2-3视图输入时重建速度达69-89 FPS，GPU内存仅203MB，比现有方法快5-107倍。

Conclusion: G2SR为在线高斯表面重建提供了一条实用路径，证明了通过结合轻量级神经网络前端与解析几何后端，可以在少视图条件下实现高效且准确的表面重建。
...
TL;DR: 本文提出首个支持无序图像序列的即时反馈3DGS重建方法，通过重利用视觉地点识别模型构建共视图实现快速匹配，并引入基于聚类的闭环优化和渐进式层次结构以处理大规模场景，在多个数据集上验证了其高效性和视觉质量。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS重建方法中，SfM需所有图像可用且计算成本高，增量式方法（如SLAM）虽可提供即时反馈但无法处理无序图像序列，缺乏一种既能提供即时反馈又能处理无序捕获并保证全局一致性的解决方案。

Method: 1) 利用视觉地点识别模型和共视图进行无序序列的快速匹配；2) 基于共视图的高连通性关键帧选取策略；3) GPU优化与高斯原语精心放置的快速局部重建；4) 基于共视图的聚类方法实现无需顺序输入的闭环优化；5) 渐进式层次结构以扩展到大规模场景。

Result: 在多个数据集上（包含多达数千张输入图像）实现了具有良好视觉质量的即时反馈3DGS重建。

Conclusion: 该方法首次实现了对无序图像序列提供即时反馈的3DGS辐射场重建，兼顾全局一致性与大规模场景的可扩展性。
...
TL;DR: 本文提出Ego Scene Augmentation (ESA)框架，通过构建Ego-element Graph来增强多模态大模型在自我中心视觉问答中的空间感知能力，在EgoTextVQA基准上室内场景提升8.14%，室外场景提升8.72%。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大模型在复杂自我中心场景中由于空间感知能力有限，难以进行有效的空间推理。

Method: 提出ESA框架，核心是利用Ego-element Graph作为中间表征，通过视觉基础模型提取并整合自我中心空间特征，从而增强MLLMs的自我中心空间感知能力。
...
TL;DR: 本文提出了DS@GT ARC在PlantCLEF 2026挑战赛中的第三名解决方案，该方案基于DINOv2 ViT-L/14分类器、多尺度图块分解、FAISS kNN检索、时间融合、栖息地适应降级和地理掩码，在仅使用单标签图像训练的条件下，实现了对高分辨率植被样地图像中多物种识别的私有排行榜macro-F1为0.43902。


<details>
  <summary>Details</summary>
Motivation: 解决植被样地图像中多物种识别问题，即在仅用单标签个体植物图像训练的情况下，预测高分辨率样地照片中存在的所有植物物种。

Method: 基于微调的DINOv2 ViT-L/14分类器，对样地图像进行多尺度图块分解，逐图块预测；结合FAISS kNN检索器进行预测融合；后处理包括：跨多次样地访问的时间融合、基于地理和海拔先验的栖息地适应降级、西南欧地理掩码。还探索了跨区域Transformer（含噪声学生蒸馏）、标签即查询的Transformer解码器、实例感知分割裁剪等方向但未获得提升。

Result: 私有排行榜macro-F1为0.43902（第三名），公开排行榜为0.51096；未选择的配置在私有集上超过0.45。消融实验表明栖息地适应降级和多尺度聚合是最大的贡献因素。

Conclusion: 基于DINOv2的多尺度图块分解结合后处理策略（栖息地适应降级、时间融合、地理掩码）能有效解决单标签训练下多物种样地识别任务，而部分复杂的训练范式（跨区域Transformer、标签查询解码器）和实例感知裁剪未能带来提升。
...
TL;DR: 提出了一种利用GPU硬件加速的纹理压缩方案（BC1/BC7）来压缩3D高斯溅射中的球谐系数，通过局部分组和重排序实现更高效的颜色系数压缩，并引入支持随机访问的码率控制策略，在保持并行渲染性能的同时显著降低内存需求。


<details>
  <summary>Details</summary>
Motivation: 3DGS中球谐系数数量庞大导致内存需求过高，现有压缩方法未充分利用现代GPU的并行解码能力和硬件加速特性。

Method: 采用GPU硬件加速的纹理压缩方案（BC1/BC7）压缩SH颜色系数；对基元进行局部分组和颜色重排序以提升压缩效率；引入支持随机访问的码率控制策略以保持并行渲染性能。

Result: BC1和BC7格式的GPU解码实现了可忽略或不可察觉的视觉质量损失，同时有效降低了内存占用。

Conclusion: 基于GPU硬件加速的纹理压缩方法比2D纹理压缩更高效，能够在保持高质量渲染的同时解决3DGS的内存瓶颈问题。
...
TL;DR: 本文提出跨场景目标导航任务及基于VLM的训练无关导航框架\method，通过持久化的分层视觉-拓扑记忆（VTM）组织场景知识，实现经验重用，在三个基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有训练无关的VLM导航方法通常在 episodic 协议下评估，每次任务后重置场景状态，无法利用跨场景经验。作者提出Cross-Episode Object-Goal Navigation任务，要求智能体在同一场景中持续运行，仅保留自获取经验，模型参数固定，以更贴近真实应用的需求。

Method: 提出\method框架，核心是持久化分层视觉-拓扑记忆（VTM），在房间级和物体级组织场景知识，通过粗到细匹配检索相关经验；仅在记忆与当前观察一致时提供软引导；配合保守执行守卫缓解振荡、阻塞和过早停止问题。

Result: 在HM3D v0.1、HM3D v0.2和MP3D三个基准上，\method均取得最佳性能，优于增强跨场景文本记忆的WMNav基线，且VLM骨干和动作管线保持一致。

Conclusion: 结构化视觉-拓扑经验重用在不同数据集上展示了有效性和鲁棒性，验证了持久化记忆机制在跨场景导航中的优势。
...
TL;DR: Uni-AdaVD是一个通用的推理时概念擦除框架，通过在多头注意力的值空间进行正交值分解与自适应擦除偏移，无需更新模型权重即可在多种视觉生成模型（U-Net、DiT、自回归图像生成器、文生视频模型）中有效擦除目标概念同时保留非目标先验。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法通常架构特定，难以在移除目标概念的同时保留非目标内容和生成先验。

Method: 将多头注意力的值空间作为统一干预空间；引入编码器感知的目标表示构建以跨异构文本编码器定位目标语义；结合正交值分解与自适应擦除偏移来抑制目标语义方向，且不更新原始模型权重。

Result: 在U-Net、DiT、自回归图像生成器以及文生视频模型上，单概念和多概念擦除均表现强劲，同时保留非目标先验。
...
TL;DR: SwinAD提出了一种基于重建的多类无监督异常检测框架，利用冻结的预训练Swin Transformer V2编码器和特征多样性保留重建解码器，通过多尺度特征和多样性重建假设提升像素级定位精度，在MVTec AD、VisA和Real-IAD基准上取得竞争性性能。
...
Method: 采用冻结的预训练Swin Transformer V2作为层次化编码器提取多尺度语义特征；每个阶段引入带dropout的瓶颈模块防止恒等映射；提出特征多样性保留重建框架，维护多个互补的重建假设而非单一解码分支；将编码器特征与两条重建特征的差异在多尺度上聚合生成最终异常图。

Result: 在MVTec AD、VisA和Real-IAD三个工业异常检测基准上，SwinAD在图像级性能上具有竞争力，在像素级定位精度上表现优异，尤其在MVTec AD上的像素级AP和PRO指标提升显著。

Conclusion: 结合层次化Swin特征与多样性多尺度重建能显著提升多类无监督异常检测场景下的像素级定位能力。
...
TL;DR: 本文提出了一种基于深度强化学习的3D牙齿正畸排齐路径自动规划框架，利用DDPG算法结合Transformer智能体、动态掩码机制和两阶段课程学习策略，在10K专家设计的数据集上实现了优于现有基线的路径安全性和几何效率。


<details>
  <summary>Details</summary>
Motivation: 现有的3D牙齿正畸排齐路径规划依赖专家手工设计，缺乏自动化和鲁棒性，需要一种能够自动生成从初始错颌到最终目标排齐的序列轨迹的方法。

Method: 将排齐规划建模为马尔可夫决策过程，采用DDPG算法，并引入三项创新：(1) Transformer智能体建模牙齿间复杂空间交互；(2) 动态掩码机制限制每步仅移动稀疏牙齿子集；(3) 两阶段课程学习策略逐步增加任务难度以稳定训练。

Result: 在10K专家设计的临床数据集上，所提方法在路径安全性和几何效率方面优于现有基线方法。

Conclusion: 该方法为3D几何正畸排齐规划提供了一种鲁棒且自动化的解决方案。
...
TL;DR: AdaTurn提出了一种预算感知框架，通过条件化智能体在允许的交互轮数上，并显式训练预算边界行为（FA-DAPO将超预算事件转化为可训练的最终决策步骤），解决了主动视觉智能体在部署时因可用预算小于偏好轨迹而导致的灾难性截断问题，在低预算场景下显著提升准确率。


<details>
  <summary>Details</summary>
Motivation: 主动视觉智能体在部署时面临不固定的交互预算：有些请求允许长轮次，有些则要求严格限制。现有方法将策略训练视为预算隐藏，导致当可用预算小于智能体偏好轨迹时，交互被截断且无法生成有效答案，即灾难性截断问题。

Method: AdaTurn框架包含三个关键组件：(1) 条件化智能体在允许的交互轮数上；(2) Forced-Answer DAPO（FA-DAPO），将超预算事件从掩码或惩罚的失败转变为可训练的最终决策步骤，使模型在无法进一步使用工具时综合部分证据；(3) 在训练和推理时随机化交互预算，并引入负载均衡调度器使操作实用化。

Result: AdaTurn在低预算场景下显著提升准确率，例如VisualProbe-Medium在4轮交互时从36.7%提升至47.6%，同时在大预算下保持强扩展性，并有效迁移到多种骨干网络和通用多模态基准。

Conclusion: AdaTurn通过显式建模预算边界行为，有效解决了主动视觉智能体在可变预算部署中的灾难性截断问题，在低预算场景下表现优异且不牺牲长预算性能，具有良好的泛化能力。
...
TL;DR: HyMobileAgent是一个基于Hy3.0-VL-A3B视觉原生基础模型的移动GUI智能体，通过联合数据和环境中心扩展框架（包括GUI感知飞轮、知识流水线、百万级动作数据流水线、PhoneWorld Mock App Factory和规划-反思机制），在三个耦合约束（精确感知、数据获取、长期决策）下实现可靠的移动界面操作。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型从内容理解转向数字环境操作，移动GUI成为具有挑战性的测试平台。移动智能体面临三个耦合约束：复杂界面的精确感知、高质量交互数据的可扩展获取、以及复合错误累积下的鲁棒长期决策。现有方法仅依赖模型规模扩展不足以解决这些瓶颈。

Method: 提出HyMobileAgent，基于Hy3.0-VL-A3B视觉原生模型（原生任意分辨率输入、A3B规模部署预算、32K上下文窗口）。构建联合数据和环境中心扩展框架，包含：(1) GUI感知飞轮（模拟界面合成、拒绝采样、图标增强）；(2) 知识流水线（教程视频转结构化交互数据）；(3) 百万级动作数据流水线（2000+沙箱和真机实例，自动故障归因）；(4) PhoneWorld Mock App Factory（34个模拟应用，34000+任务）；(5) 结构化规划-反思机制（显式死循环检测）。训练采用三阶段渐进式配方：中期训练、监督微调、任务特定奖励的强化学习。

Result: 论文报告了HyMobileAgent在移动GUI智能体任务上的性能表现，但摘要未给出具体数值结果。

Conclusion: 联合数据和环境中心扩展框架比单纯模型扩展更能有效突破移动交互的关键瓶颈，通过多维度数据/环境工程和结构化推理机制实现可靠的移动GUI操作。
...
TL;DR: 本文提出LDMR框架，通过监控检测质量的**学习动态**信号，驱动**阶段性复习**与**跨阶段记忆演化**机制，有效缓解增量3D目标检测中的灾难性遗忘问题，在长序列增量场景下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于伪标签的增量3D检测方法在长增量序列中遭受严重的模型遗忘，原因在于**自增强恶性循环**：新类数据分布偏移导致旧类遗忘，伪标签积累误差进一步加速模型退化。

Method: 提出LDMR框架，包含两个核心机制：(i) **类内阶段性复习**——将每个增量阶段划分为多个子阶段，根据学习动态信号重点复习遗忘最严重的类别；(ii) **场景感知的跨阶段记忆演化**——联合考虑场景可学习性与多样性，演化记忆库以在连续阶段间传递知识。

Result: 在SUN RGB-D和ScanNetV2室内基准的多个长增量协议上，LDMR显著缓解了模型遗忘，以明显优势超越所有基线方法。

Conclusion: LDMR通过模拟人类学习过程中的复习与记忆演化机制，有效打破了增量学习中的自增强遗忘循环，为长序列增量3D目标检测提供了新思路。
...
TL;DR: 本文提出了一种结合负提示优化（通过微调序列到序列LLM）和潜在空间分类器引导的双重引导框架，用于提升Stable Diffusion生成图像的质量，实验表明该方法能减少伪影并提高语义保真度。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型生成的图像常存在伪影和语义不匹配问题，需要一种自动化的方法来优化负提示并引导扩散过程，以提升生成质量。

Method: 1) 使用微调的序列到序列LLM自动生成优化的负提示；2) 采用CNN-RNN混合分类器在潜在空间中对扩散步骤进行评估和引导，回滚低质量的潜在更新。

Result: 与基线扩散模型相比，该双重引导框架减少了伪影并提高了语义保真度。

Conclusion: 提出的负提示优化与潜在空间分类器引导的联合方法能有效提升Stable Diffusion的图像生成质量。
...
TL;DR: MagicPrompt是一个轻量级框架，通过注意力嵌入提示调优（仅需不到1%的可训练参数）和双空间奖励反馈优化（提升条件引导任务中的奖励训练稳定性），解决了大规模视频扩散模型微调中计算成本高、参数效率低和奖励优化不稳定的问题。


<details>
  <summary>Details</summary>
Motivation: 大规模视频扩散模型在下游任务全微调时计算成本过高，现有参数高效微调方法在十亿级模型上仍需要大量可训练参数，且基于奖励的训练在条件引导任务中存在噪声导致的优化不稳定性。

Method: 提出MagicPrompt框架，包括两个核心组件：(1) 注意力嵌入提示调优（Attention-Embedded Prompt Tuning），通过轻量级软提示以数量级更少的参数引导生成，同时保留预训练知识；(2) 双空间奖励反馈优化（Dual-Space Reward Feedback Optimization），利用自监督潜在目标改进条件引导奖励训练。

Result: MagicPrompt在不到1%的可训练参数下达到与全微调相当的竞争性能，并显著降低了训练成本。

Conclusion: MagicPrompt实现了极端的参数效率和稳定的奖励优化，为大规模视频扩散模型的高效微调提供了有效方案。
...
TL;DR: 针对SIFT在强线性结构场景中注册失败的问题，提出Hough-SIFT方法，通过在Hough空间进行描述子匹配，利用线性结构在霍夫空间形成独特峰值的特性恢复描述子辨别能力，在保持正常场景精度的同时显著提升了线性场景的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: SIFT在具有强线性结构（如百叶窗）的场景中，局部特征变得模糊，导致注册失败。现有方法缺乏对这类特殊场景的鲁棒处理能力。

Method: 将SIFT描述子匹配迁移到Hough空间中进行。在线性场景中，线性结构在Hough空间形成独特的峰值，从而恢复描述子的可区分性。

Result: Hough-SIFT在线性场景中显著优于SIFT，鲁棒性强；在正常场景中保持与SIFT相当的精度。

Conclusion: Hough-SIFT有效解决了SIFT在线性结构场景下的注册失败问题，同时不牺牲正常场景的性能，是一种鲁棒的图像注册方法。
...
TL;DR: GlanceFace是一个端到端框架，利用视觉-语言模型和语义增强的面部表征，从面部图像推断表面人格（MBTI类型），并通过不确定性感知学习策略处理噪声标注，在表面人格推断基准上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有表面人格推断研究主要聚焦大五人格模型且依赖语言或多模态输入，尚不清楚仅凭面部线索能否支持有意义的MBTI人格感知关联，而MBTI在实际应用和LLM解释性方面更有优势。

Method: 提出GlanceFace框架：1）利用视觉-语言模型引入语义先验；2）设计语义增强的面部表征模块捕捉细微人格线索；3）采用不确定性感知学习策略处理噪声和主观标注。

Result: 在基于MBTI的表面人格推断基准上取得强性能，揭示了面部特征与感知人格特质之间的关系。

Conclusion: GlanceFace证明了仅凭面部线索推断MBTI人格类型的可行性，为具身智能体在人机交互中制定自适应初始交互策略提供了潜在支持。
...
TL;DR: FilmGPT是一个自回归Transformer模型，通过从大量电影数据中隐式学习电影语法，利用镜头约束解码算法从原始素材中选择最佳镜头序列，实现自动视频剪辑，无需生成新视频帧。


<details>
  <summary>Details</summary>
Motivation: 解决视频蒙太奇（将原始、"不可看"的素材片段转化为连贯电影序列）的自动化挑战，避免依赖手工编码规则，让模型从数据中隐式学习电影的"语法"。

Method: 训练一个长上下文自回归Transformer（FilmGPT）在大规模电影语料上；推理时提出镜头约束解码算法，根据从电影中学到的统计模式从输入原始素材中选择最佳下一个镜头。

Result: 在镜头序列排序基准上，FilmGPT自回归模型的下一镜头预测超越之前的最先进方法；通过用户研究，基于FilmGPT的剪辑算法在完整电影编辑任务上显著优于先前方法。

Conclusion: FilmGPT可广泛应用于视频蒙太奇任务，从自动视频片段修剪到人在环路的电影剪辑，展示了数据驱动学习电影语法的有效性。
...
TL;DR: VIABench是一个专门用于评估多模态大语言模型在视障辅助场景中表现的视频基准测试，包含主动提醒、视觉问答和视觉引导交互三大核心任务，实验表明现有MLLMs仍难以提供全面支持。


<details>
  <summary>Details</summary>
Motivation: 视障人士在日常生活中面临获取视觉信息的重大挑战，尽管多模态大语言模型在通用视觉和语言任务上表现优异，但其在真实盲人辅助场景中的实用性尚未得到充分探索。

Method: 构建了VIABench基准，使用视障人士自己录制或分享的第一人称视频，定义了主动提醒、视觉问答和视觉引导交互三个核心任务，并设计了支持在线和离线设置的严格评估流程。

Result: 当前多模态大语言模型在视障辅助场景中仍难以提供全面支持，尤其在主动提醒任务中表现不佳，该任务需要准确的预测能力和实时响应能力。

Conclusion: VIABench将推动未来研究开发定制化的多模态大语言模型用于真实世界辅助，从而改善视障人士的导航和交互体验。
...
TL;DR: 提出ReBind框架，通过嵌入参考令牌的语义指令作为中间表示，解决多参考图像条件视频编辑中多视觉源信息协调不准确的问题，在指令质量和编辑性能上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以准确协调多个视觉源的信息，原因是编辑指令缺乏显式的引用关系，且大多数多模态大语言模型无法可靠生成这类关系。

Method: 提出ReBind框架：(1) ReBind-Instruct：专门设计的MLLM，通过两阶段渐进式方案学习建立视觉属性与参考源之间的显式绑定；(2) ReBind-Edit：实现文本到视频模型的轻量级适应，通过将视觉属性绑定到指定源来协调多个参考。

Result: 在指令质量上显著优于通用MLLM，在参考图像条件视频编辑任务中达到开源方法中的SOTA性能。

Conclusion: 嵌入参考令牌的语义指令作为中间表示能有效消除歧义，建立视觉属性与其来源之间的精确绑定，是多参考视频编辑的有效范式。
...
TL;DR: 现有AIGI检测器对真实世界传播退化（如JPEG压缩、模糊）鲁棒性差，原因是它们过拟合了局部伪影。本文提出GlobalForge框架，通过局部信息瓶颈（LIB）抑制局部捷径学习、全局结构推理（GSR）模块强制远程证据聚合，并结合对比结构损失，在8个野外基准组上平均BAcc提升5.89%，同时在提出的RealDeg-Bench（含7种单一及复合退化）上显著优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有AIGI检测器在干净基准上表现优异，但经真实传播通道（如JPEG压缩、模糊）后性能急剧下降。作者追溯发现，检测器过拟合了生成器在局部空间邻域留下的脆弱伪影，这些伪影易被常见传播退化破坏。

Method: 提出GlobalForge框架，包含两个互补模块：1) 局部信息瓶颈（LIB）抑制局部组件以阻断捷径学习；2) 全局结构推理（GSR）模块强制每个token从远距离区域收集证据。两个模块在基于退化的对比结构损失下联合训练，使特征在退化下保持稳定。同时引入RealDeg-Bench基准，覆盖7种常见退化算子及多步复合链。

Result: GlobalForge在8个野外基准组上平均BAcc提升5.89%，在RealDeg-Bench的单一和复合退化设置下均明显领先于代表性基线。

Conclusion: 将检测线索从脆弱的局部伪影转向鲁棒的全局结构，能有效提升AIGI检测器在真实世界传播退化下的泛化能力。
...
TL;DR: 本文提出一种以文本为中心的文本残差融合模型，用于ABAW挑战中的矛盾与犹豫识别，通过将文本作为锚定模态并利用门控残差调整融合声学、面部和场景特征，在BAH语料库上取得75.14%的平均MF1（开发/公开测试集）和78.24%的MF1（私有测试集），比纯文本模型提升4.03%，证明多模态信息无需大模型集成即可提升性能。


<details>
  <summary>Details</summary>
Motivation: 矛盾与犹豫的自动识别具有挑战性，因为这些状态可能通过不一致的语言、声学、面部和上下文模式表达，而当前顶尖系统往往依赖计算昂贵的集成模型，因此需要一种高效且无需大型集成的单模型多模态方法。

Method: 提出文本中心多模态方法（Text Residual Fusion），将文本作为锚定模态，基于其他模态（声学、面部、场景）的特征通过门控残差调整机制对文本特征进行修正融合，实现视频级别的矛盾与犹豫识别。

Result: 在BAH语料库上，文本残差融合模型在开发集和公开测试集上平均MF1达75.14%；在私有测试集上MF1达78.24%，比纯文本模型提升4.03%。

Conclusion: 互补的多模态信息能够在不依赖大型模型集成的情况下有效提升矛盾与犹豫识别的性能，验证了以文本为中心的门控残差融合策略的有效性。
...
TL;DR: 本文提出了一种基于知识蒸馏的MIL预训练框架，利用两个WSI级别的基座模型（TITAN和CARE）作为教师，将表征知识迁移到轻量级MIL聚合器中，并引入角度分散归一化蒸馏损失来平衡不同教师的监督信号。在15个基准数据集上的实验表明，该预训练方法相比从头训练能显著提升MIL模型性能，尤其是在线性探测和少样本场景下。


<details>
  <summary>Details</summary>
Motivation: 现有的MIL聚合器通常从零开始针对每个下游任务训练，仅依赖有限的切片级标签同时学习聚合机制和判别性表示，导致优化不稳定、过拟合以及迁移性差。缺乏高质量的切片级预训练数据和可复用的预训练初始化。

Method: 提出蒸馏式预训练框架，使用TITAN和CARE两个切片级基座模型作为教师，将知识蒸馏到多种MIL架构中；引入角度分散归一化蒸馏损失以平衡不同教师的监督信号。蒸馏后的权重作为下游任务的初始化。

Result: 在15个基准数据集上系统评估，表明预训练普遍优于从头训练，尤其在线性探测和少样本设置下效果显著，同时保持轻量级MIL模型的计算效率。

Conclusion: MIL预训练框架有效解决了现有MIL从头训练的局限性，提升了模型性能、稳定性和迁移能力，为计算病理学中的MIL提供了可复用的预训练初始化范式。
...
TL;DR: 本文提出TVB网络，将变分推断框架引入BEV分割，利用条件变分自编码器（CVAE）结合归一化流生成多个候选BEV地图，并通过BEV注意力融合（BAF）模块自适应集成这些候选图，在nuScenes和OPV2V数据集上实现了多相机BEV分割和车道环境感知的最优性能。


<details>
  <summary>Details</summary>
Motivation: BEV在自动驾驶中至关重要，但多相机数据融合及复杂外部环境下的感知仍是重大挑战，现有方法难以有效解决这些问题。

Method: 将BEV分割重新定义为变分推断问题，提出基于Transformer的变分流变换网络（TVB）。采用条件变分自编码器（CVAE）作为骨干网络生成多个候选BEV图，融合归一化流增强概率分布的复杂性和表达能力，并设计BEV注意力融合（BAF）模块利用注意力机制自适应集成多个候选BEV图。

Result: 在nuScenes和OPV2V数据集上的实验表明，该方法在多相机BEV分割和车道环境感知任务中取得了优越性能。

Conclusion: TVB通过变分推断框架有效解决了多相机BEV融合难题，归一化流与注意力融合机制的引入显著提升了BEV分割的准确性和鲁棒性。
...
TL;DR: 提出VideoSEMA模型，采用分体时空注意力机制（空间SEMA块+时间softmax注意力），在视频理解任务中以更低计算成本实现与全局时空注意力等价的效果，在K400和SSv2数据集上超越ViT和Mamba模型，且在高分辨率下表现更稳定。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解模型（如Vision Transformer和VideoMamba）计算成本高，且在高分辨率或长视频场景下性能退化严重。作者希望设计一种既高效又可扩展的时空注意力机制，在保持较低计算量的同时达到或超越全局注意力的效果。

Method: 提出VideoSEMA模型，包含：(1) 空间维度上的SEMA块（Mamba-like架构），并行执行局部窗口注意力和全局平均池化；(2) 时间维度上的softmax注意力。理论证明在特定秩条件下，这种分体时空注意力与全局时空注意力等价。

Result: 在K400数据集上超越更重的ViT和Mamba模型；在SSv2数据集上，在相似参数量模型中达到最高top-1准确率；当分辨率从224²升至1024²时，性能退化远小于VideoMamba。

Conclusion: 分体时空注意力在计算效率和性能之间取得了更优平衡，且可扩展至更高分辨率和更长视频（通过膨胀/稀疏时间注意力）。
...
TL;DR: 本文提出一种因果推理框架，利用对抗性协变量抑制方法分析前列腺MRI深度学习分级模型中临床协变量的作用，区分其是有用的疾病信号还是误导性的捷径信息。在2903例患者上的实验表明，抑制年龄、BMI、饮酒量等协变量可提升模型泛化性能（AUC提升0.84%-1.42%），而抑制PSA和前列腺体积则会显著降低性能（AUC下降1.91%-7.61%），表明前者是非泛化的捷径信息，后者携带任务相关信号。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在前列腺MRI癌症分级中可能编码临床协变量，但这些协变量究竟反映有意义的疾病信号还是非泛化的捷径信息通常只是被假设而未被验证。因此需要一种系统的因果推理方法来探测和区分不同协变量的作用。

Method: 提出因果推理框架：将mpMRI外观和ISUP分级视为潜在肿瘤病理的观测变量，而非将MRI直接视为分级的因。采用对抗性框架，在保持MRI分级预测能力的同时，逐个抑制模型对特定临床协变量的解码能力，通过性能变化判断各协变量的作用类别（ nuisance correlate/disease-related proxy/irrelevant covariate）。

Result: 在2903例（内部）和576例（外部验证）前列腺MRI数据上，二分类ISUP Grade Group ≥2任务中：抑制年龄、BMI、饮酒量分别使AUC提升1.23%、0.84%、1.42%（p<0.05），表明这些是非泛化的协变量信息；抑制PSA和前列腺体积分别使AUC下降1.91%和7.61%（p<0.001），表明它们携带任务相关信号。

Conclusion: 对抗性协变量抑制方法能够提供实用的表示级分析，有效区分前列腺MRI分级模型中潜在的危害性依赖（捷径信息）与有信息量的信号，从而指导模型泛化性改进。
...
TL;DR: 本文提出测试空间训练（TST），通过在测试环境中收集多模态数据并进行自监督预训练，在小规模领域特定数据上取得了与互联网级预训练模型（如DINOv2、CLIP）相竞争的性能，揭示了多模态数据可以替代大规模数据的可能性。


<details>
  <summary>Details</summary>
Motivation: 探索在受限测试环境中，仅利用设备自身多模态传感器采集的数据进行跨模态自监督学习，能否达到与互联网大规模预训练模型相当的性能，从而减少对外部大规模数据集的依赖。

Method: 提出Test-Space Training (TST)框架：在特定测试环境中收集多模态数据，进行自监督跨模态预训练，然后在下游任务上微调评估。通过限制设备仅在一个测试环境中操作，形成专业化（specialization）学习范式。

Result: 1) 仅使用测试环境的多模态数据，TST可以达到与DINOv2、CLIP等互联网规模预训练模型竞争的准确率；2) 多模态性可以替代数据量，减少对外部大规模数据集的需求；3) 预训练数据的差异在模型专业化和泛化能力之间存在权衡。

Conclusion: 跨模态学习在受限测试环境中是有效的自监督范式，多模态数据可以作为大规模数据的替代品。TST为资源受限设备提供了不依赖互联网数据的预训练新路径，但专业化和泛化能力之间存在权衡关系。
...
TL;DR: 本文提出了AE-UAV——首个空中拍摄的空对空（A2A）事件相机无人机跟踪基准数据集，包含178个飞行序列和连续时间三次B样条标注；同时提出了快速-慢速频域跟踪（FSFT）方法，该方法轻量级、无需训练，在仅CPU硬件上以420 FPS运行，相比GPU依赖的SOTA方法保持93.97%精度并实现5.32倍有效加速，为机载遥感提供了高效鲁棒的解决方案。


<details>
  <summary>Details</summary>
Motivation: 传统帧相机在低照度、过曝光和高速度运动下性能严重下降；事件相机虽有微秒级时间分辨率和高动态范围优势，但缺少专用的A2A事件基数据集，且现有跟踪器严重依赖GPU加速和大量训练数据，无法应用于资源受限的无人机平台。

Method: 构建了AE-UAV数据集（首个空中拍摄的A2A事件相机跟踪基准，178个飞行序列，连续时间三次B样条标注）；提出了FSFT（Fast-Slow Frequency-domain Tracking）方法，该轻量级无训练框架结合了频域模板匹配、搜索区域预测和基于检测的漂移校正。

Result: FSFT在仅CPU硬件上达到420 FPS超快速度，保持GPU依赖的SOTA方法93.97%的精度，实现5.32倍有效加速，并展现出优越的时间分辨率泛化能力。

Conclusion: AE-UAV数据集和FSFT方法填补了A2A事件相机跟踪领域的空白，为资源受限无人机平台上的连续实时空中目标跟踪提供了高效鲁棒的解决方案，数据与代码已开源。
...
TL;DR: WorkDrive是一个面向自动驾驶视觉语言模型的框架，通过构建面向施工区域的感知因果推理并与轨迹预测对齐，在ROADWork数据集上使轨迹平均位移误差降低9.0%，并在此基础上通过一致性强化学习进一步降低3.0%。


<details>
  <summary>Details</summary>
Motivation: 现有的自动驾驶VLM在施工区域表现不佳，因为熟悉的视觉线索（如车道线、永久标志）缺失或被改变，而临时设备（如锥桶、护栏）重新定义了可行驶区域。VLM虽能检测这些物体，但缺乏明确引导时会依赖预训练中的熟悉元素，无法将施工区域观测与正确的规划决策联系起来。

Method: WorkDrive框架包含：1) 自动多任务感知pipeline，提取结构化场景事实；2) 将这些事实注入因果链（Chain-of-Causation, CoC）标注pipeline，将标注注意力重定向到领域特定元素；3) 使用生成的推理标签进行监督微调；4) 基于横向元动作与预测轨迹一致性的单一奖励进行强化学习（GRPO）。

Result: 在最大的公开施工区域数据集ROADWork上，提出的施工区域CoC使轨迹平均位移误差（ADE）降低9.0%，基于一致性的GRPO进一步降低3.0%，实现了相比纯轨迹基线的渐进式改进。

Conclusion: 通过感知因果推理引导和一致性强化学习，WorkDrive有效提升了VLM在施工区域的轨迹预测能力，代码和数据将公开发布。
...
TL;DR: VQ-Touch提出了一种新颖的触觉图像生成框架，通过DM-VQGAN高效提取变形与纹理特征，并利用离散扩散解码器和统一条件接口实现跨传感器与多场景的触觉生成，在多项任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有触觉图像生成方法严重依赖特定传感器的大规模多样化数据集，数据利用效率低且泛化能力不足，尤其在视觉受限环境中表现不佳，因此需要一种更高效、跨传感器且具备强泛化能力的触觉生成框架。

Method: 提出VQ-Touch框架，包含两个核心模块：1) DM-VQGAN，一种高效的触觉表示学习器，用于提取复杂的变形和纹理特征；2) 离散扩散解码器，配备统一条件接口，支持图像和标签等多模态生成任务，并通过少样本混合训练增强模型泛化能力，实现与主流传感器及其变体的兼容。

Result: 实验结果表明VQ-Touch在多个任务上超越了当前最先进的方法。

Conclusion: VQ-Touch通过高效的触觉表示学习和离散扩散机制，成功实现了跨传感器、多场景的触觉图像生成，为机器感知与人机交互系统中的触觉信息获取提供了更通用、高效的解决方案。
...
TL;DR: 本文针对多模态视觉-语言预训练模型（VLPs）的对抗样本检测问题，发现VLP嵌入空间存在不同于单模态视觉模型的各向异性结构，并理论证明在此结构下对抗攻击会增加干净样本与对抗样本的几何分离度。据此提出GeoDetect方法，利用几何分数检测偏离流形的对抗样本，在多种VLP架构和攻击场景下均表现鲁棒。


<details>
  <summary>Details</summary>
Motivation: 现有对抗检测方法在单模态（视觉或语言）中表现良好，但在多模态VLP模型中的有效性和可靠性尚未得到充分探索。

Method: 首先研究VLP嵌入空间的几何特性，发现其存在各向异性结构；然后理论证明在此结构下，对抗攻击会增加干净样本与对抗样本到随机采样点的期望几何距离，即对抗样本倾向于将表示推出流形区域；最后提出GeoDetect方法，通过几何分数（几何距离度量）来识别偏离流形的对抗样本。

Result: GeoDetect在多种VLP架构和威胁场景下均能可靠检测对抗样本，涵盖单模态攻击、多模态攻击以及自适应攻击。

Conclusion: GeoDetect为提升VLP模型的安全性和可靠性提供了一种鲁棒且实用的对抗检测方案。
...
TL;DR: FoMoVLA通过联合学习未来特征预测和稀疏2D点追踪，增强VLA模型的时空监督，实现状态预测与运动路径的互补，在多个基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型本质上是反应式的，缺乏对世界动力学的显式前向预测；视觉预见方法能预测未来状态但缺乏运动引导。作者认为未来特征预测和稀疏点追踪天然互补，应结合两者以增强动作策略。

Method: 提出FoMoVLA框架：1) 引入紧凑的预见令牌解码未来特征状态；2) 解码稀疏时序2D点轨迹建模紧凑几何运动；3) 通过轻量级未来条件化的交叉注意力模块耦合两者，实现状态与点动力学间的一致推理。

Result: 在LIBERO、RoboCasa GR-1 Tabletop、LIBERO-Plus三个基准上取得SOTA性能，并展现出强零样本泛化能力。

Conclusion: 未来特征预测与稀疏点追踪的联合学习能有效增强VLA的连续动作策略，两者互补的设计在机器人操作任务中表现优异。
...
TL;DR: 本文系统研究了基于扰动的可解释AI方法中两个关键参数（补丁几何形状和扰动类型）对洪水检测任务中相关性估计的影响，发现不同参数设置会导致模糊甚至矛盾的显著性图，强调方法学参数选择的重要性。


<details>
  <summary>Details</summary>
Motivation: 基于扰动的xAI方法虽然被广泛使用，但其结果对参数选择高度敏感，现有研究缺乏对补丁几何形状（大小、形状）和扰动类型（替换方案）这两个核心参数如何影响相关性估计的系统性理解。

Method: 以合成孔径雷达（SAR）影像洪水检测为应用场景，通过改变补丁大小、形状和扰动类型等参数，生成多组相关性图，结合视觉检查和定量指标（一致性和忠实度）评估不同扰动策略下的解释结果。

Result: 不同扰动参数选择会显著改变生成的相关性图，产生模糊甚至相互矛盾的解释结果；参数的微小变化可能导致显著性图的根本性差异。

Conclusion: 扰动参数选择是xAI方法中的关键组成部分，必须在解释模型时将其作为整体要素进行仔细检查和评估，以确保对解释结果和模型预测的稳健理解。
...
TL;DR: 本文提出了一种针对监控摄像头的流式完整性监测方法，通过分析亮度与局部梯度统计量，检测纹理丢失性遮挡和光照剧变两种传感器故障，并采用状态机控制告警频率，在多个数据集上验证了低误报率和可控的诊断性能。


<details>
  <summary>Details</summary>
Motivation: 监控摄像头的物理退化会无声地影响所有下游数据消费者，现有方法缺乏低误报率、有限计算和可诊断性的综合保障，特别是在光照变化等干扰下表现不佳。

Method: 方法包括：比较采样亮度和局部梯度统计量与干净滑动参考；应用粗网格结构光抑制和快速亮度变化抑制模式；采用状态机确保每个篡改事件最多发出一次通知；形式化决策谓词并推导快速亮度抑制使场景转换路径不可达的一致性规则。

Result: 在320个受控序列上达到0.800 F1和0.822平衡准确率；在公共审计数据集上达到最高partial AUC（5%误报预算）；在扩展压力测试中达到0.925召回率（0.025误报率）；在Xiph、Bremen IoT和UHCTD数据集上保持低误报，UHCTD范围内召回率0.667 vs 范围外0.016；9.09摄像头小时验证阴性审计零误报。

Conclusion: 该方法更适合作为可审计的传感器健康子系统而非通用摄像头篡改分类器，在限定篡改特征下具有高可靠性和低误报率。
...
TL;DR: 提出了一种基于反事实推理的扩散模型CI-Diff，通过阻断模型固有常识偏见的干扰，利用自然直接效应解耦罕见概念中的非典型属性，在RareBench基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在生成罕见概念（具有非典型属性的物体）时存在常识偏见，导致属性渲染错误或概念组合不一致。

Method: 提出CI-Diff方法，利用反事实推理阻断模型固有常识偏见的干扰，通过自然直接效应（Natural Direct Effect）捕捉文本提示对图像生成的独立影响，从而解耦罕见概念中的异常属性；并重构无分类器引导机制以突出非典型属性。

Result: 在RareBench基准上的大量实验表明，CI-Diff显著优于现有的最先进扩散模型。

Conclusion: 首次将因果推理引入罕见概念生成任务，有效解决了扩散模型中的常识偏见问题，实现了更准确的罕见概念图像生成。
...
TL;DR: 提出TAMF-VTON，一种无需分割掩码的纹理感知虚拟试穿框架，支持多服装同时转移，通过MoE适配、频域监督和反VTON数据增强实现高质量合成，在消费级硬件上15秒内完成推理。


<details>
  <summary>Details</summary>
Motivation: 现有扩散式虚拟试穿方法受限于依赖分割掩码、细粒度纹理保持不足、仅支持有限的多服装组合，难以在实际电商场景中部署。

Method: 提出三部分统一生成管线：(1)轻量级MoE适配实现高效微调；(2)频域监督机制优化高频光谱一致性以保持纹理；(3)自适应修补策略模拟反VTON过程以生成高质量训练数据。

Result: 在定量指标和感知质量上均超越现有方法，在RTX 4090上INT4量化后每张图像推理时间低于15秒。

Conclusion: TAMF-VTON通过无掩码操作、灵活多服装组合、忠实纹理保持和高效推理，提供了可规模化部署的商用虚拟试穿解决方案。
...
TL;DR: 本文提出了一种用于RoboCup比赛的自主实时解说系统，通过神经符号架构将运动跟踪数据转化为流畅、无幻觉的自然语言评论，可同时应用于直播和赛后分析，并适应不同尺寸人形机器人同场竞技的新动态。


<details>
  <summary>Details</summary>
Motivation: RoboCup社区需要持续衡量研究进展，而计算比赛视频的视觉统计信息是跟踪这一演进的关键方式，但目前缺乏能够实时、自主生成高质量评论的系统。

Method: 采用神经符号（neuro-symbolic）架构，桥接原始运动跟踪数据与自然语言生成之间的鸿沟，从视频流中提取精确统计信息并转化为流畅、无幻觉的叙述。

Result: 系统能够在直播流和赛后分析中实时生成统计数据和评论，且能适应不同尺寸人形机器人同场竞技的新联赛动态。

Conclusion: 该系统展示了在RoboCup场景下实现全自主、实时解说的可行性，为衡量研究社区进展提供了新的工具，有望推动RoboCup向2050年对战人类世界杯冠军的目标迈进。
...
TL;DR: 本文综述了行人重识别（ReID）从单模态RGB向跨模态与多模态范式的演进，系统梳理了可见光-红外、文本-图像、素描、非视距（NLOS）及三光谱/多模态融合ReID等任务，总结了数据集、挑战与方法，并提出了一个基于Transformer的可见光-红外ReID基线框架，最后展望了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 传统单模态RGB ReID受限于低光照、遮挡等环境挑战，亟需向跨模态与多模态方向演进以提升鲁棒性，但目前缺乏对此转变的系统性综述。

Method: 通过系统综述梳理可见光-红外（VI-ReID）、文本-图像（TI-ReID）、素描（Sketch-ReID）、非视距（NLOS）ReID及三光谱/多模态融合ReID等任务，并提出基于Transformer的VI-ReID基线框架以捕获模态不变特征。

Result: 总结了各类跨模态/多模态ReID任务的数据集、挑战与现有方法，提出了Transformer基线框架用于可见光-红外ReID。

Conclusion: 跨模态与多模态ReID是解决传统单模态局限性的重要方向，Transformer架构有助于提取模态不变特征，未来研究可沿综述指出的方向继续深入。
...
TL;DR: PIDiffSign是一种物理信息引导的扩散模型，通过在架构和训练目标中融入解剖学约束（骨长一致性、关节角有效性等），在词汇到姿态的符号语言生成任务中同时提升了语义 fidelity 和运动真实性，在PHOENIX14T和CSL-Daily基准上优于强扩散基线。


<details>
  <summary>Details</summary>
Motivation: 现有方法将骨架视为非结构化向量进行坐标级优化，导致骨长漂移、关节角违规和手指锁定等问题，无法同时满足语义保真度和生物力学合理性两个约束。

Method: 采用Transformer编码器-解码器架构，解码器通过自适应零初始化层归一化（Adaptive Zero-Initialized Layer Normalization）条件化扩散时间步，并与词汇表示进行交叉注意力。集成可微几何模块保证骨长一致性和生物有效关节角。训练损失融合人类形态学、运动学、角度和指关节约束，加上对比性词汇-姿态对齐损失和无分类器引导。

Result: 在PHOENIX14T和CSL-Daily基准上，在姿态精度、关节角正确性、分布真实性和反向翻译质量上均一致优于强扩散基线。

Conclusion: 物理信息扩散方法能同时提升符号语言生成的运动真实性和语义保真度。
...
TL;DR: 本文提出一种基于旋转运动补偿的相移轮廓术框架，用于动态3D眼球重建。通过球坐标用户特定3D眼模型估计帧间旋转，补偿像素失配和相移误差，并采用区域优化策略减少残余伪影。实验证明该方法能有效抑制运动引起的形变，提升重建精度，且不局限于球形几何。


<details>
  <summary>Details</summary>
Motivation: 传统2D图像法进行眼动追踪在稳定性、精度和鲁棒性上存在局限；3D眼表重建可提供更丰富的几何信息，但相移轮廓术（PSP）在眼球旋转时易产生帧间运动误差，导致重建形变。

Method: 提出旋转运动补偿框架：1）基于图像运动线索，利用球坐标下用户特定3D眼模型估计相对眼球旋转；2）利用估计的运动补偿帧间旋转导致的相机像素失配和相移误差；3）引入区域优化策略，对不同眼区独立调整补偿强度以减少残余伪影。

Result: 在非均匀旋转的仿生眼实验中，该方法显著抑制了运动引起的变形，提高了重建精度；在非球面刚性物体实验中，证明补偿原理不限于球形眼几何。

Conclusion: 该方法为PSP动态3D眼球重建提供了实用基础，有望推动未来沉浸式环境中的高精度眼动追踪。
...
TL;DR: 本文提出面向视觉地点识别（VPR）的按地点（place-wise）数据集剪枝框架，通过引入地内多样性（IPD）和地间相似性（IPS）两个互补指标评估每个地点的训练价值，构建紧凑且信息丰富的核心集，在显著降低存储和训练成本的同时保持VPR模型的鲁棒性和判别性。实验表明该方法在各种剪枝比例下均优于现有数据集剪枝方法，在缩减约3.5倍GSV-Cities规模的数据集上仍能达到94.5% R@1（MSLS-val）和97.0% R@1（Nordland）。


<details>
  <summary>Details</summary>
Motivation: 现有VPR研究依赖大规模数据集，带来高昂的存储和训练成本，尤其在迭代开发新架构或训练策略时。传统数据集剪枝方法基于样本级分类范式，忽略了VPR中监督信号由图像对（而非独立图像）构成的关系依赖训练特性。

Method: 提出按地点（place-wise）的数据集剪枝框架，以每个地点（place）而非单张图像作为剪枝基本单元；设计两个互补指标：地内多样性（IPD）衡量地点内部图像的多样性，地间相似性（IPS）衡量不同地点间的相似度；联合两个指标对所有地点排序，选取高训练价值的地点构建紧凑核心集。

Result: 在各种剪枝比例下一致优于现有SOTA数据集剪枝基线，同时降低选择和训练成本；在合并约3.5倍GSV-Cities规模的数据集剪枝至可比规模后，仅用NetVLAD即达到94.5% R@1（MSLS-val）和97.0% R@1（Nordland）。

Conclusion: 按地点剪枝框架能有效适应VPR的关系依赖训练特性，在保持模型性能的同时显著提升数据效率，降低存储和训练开销。
...
TL;DR: FlashDecoder 是一种快速、内存高效的全Transformer视频解码器，通过逐帧解码和滚动KV缓存机制，在保持与卷积解码器相当的重建质量（如1080p下41.55dB vs 41.49dB PSNR）的同时，实现3.6x-4.7x的解码加速和高达11x的内存节省，经架构感知优化后加速比可达12x。


<details>
  <summary>Details</summary>
Motivation: 当前潜变量视频扩散模型依赖3D卷积解码器，在高分辨率或长视频场景下解码速度慢且内存占用高，无法满足实时视频生成对快速解码和快速去噪的双重要求。

Method: 提出纯Transformer架构的FlashDecoder，采用逐帧解码方式：每帧仅通过滚动KV缓存关注固定大小的历史帧窗口，保持解码延迟和内存消耗与视频长度无关，实现恒定延迟流式处理；帧顺序处理天然保证时间因果性，无需显式注意力掩码。

Result: 在Wan2.1和Wan2.2潜空间上，FlashDecoder重建质量与卷积解码器相当（1080p下PSNR 41.55dB vs 41.49dB），解码速度提升3.6x-4.7x，内存减少高达11倍，架构感知优化后加速比达12x，且支持最高1080p分辨率训练。

Conclusion: FlashDecoder通过纯Transformer逐帧解码和滚动KV缓存机制，在保持重建质量的前提下显著提升了视频解码的速度和内存效率，为实时视频生成提供了可行的解码方案。
...
TL;DR: TanGO提出了一种免训练的3D生成编辑框架，通过在每个token的切线空间中进行自适应导向控制，解决了现有流匹配模型在语义编辑时产生的结构伪影问题。


<details>
  <summary>Details</summary>
Motivation: 现有流匹配3D生成模型（如VecSet）使用结构化表示但token共享全局上下文，导致免训练编辑出现语义伪影——被保留区域坍塌或目标区域变换不完整。

Method: 在生成动力学的切线空间中实现逐token自适应导向：1）提出一步最优控制规则；2）利用von Mises-Fisher启发的方向差异量，根据源和目标速度场确定每个token控制信号的强度。

Result: 显著减少了结构伪影，在3D编辑任务上达到了最先进的性能，超越了已有的3D编辑基线方法。

Conclusion: TanGO通过切线空间中的逐token自适应控制，有效实现了免训练的高质量3D语义编辑，代码已开源。
...
TL;DR: 本文研究了合成人脸数据集能否替代真实基准用于人脸识别评估。通过12个合成数据集、7个真实基准和24个预训练模型的系统测试，发现MorphFace和Vec2Face等高质量合成数据集能够复现真实基准的相对表现，其一致性已达到真实基准之间的自然差异水平，说明合成数据可以支持可靠的人脸识别比较评估，推动全合成、隐私保护的训练与评测流程。


<details>
  <summary>Details</summary>
Motivation: 合成人脸数据集在训练人脸识别模型方面已取得与真实数据相当的效果，避免了收集真实生物数据的伦理和法律负担。然而，现有研究即使在训练阶段完全使用合成图像，仍依赖真实人脸基准来评估性能，隐私问题仅被部分解决。因此，本文旨在探究合成数据集能否替代真实基准进行人脸识别评估。

Method: 测试12个合成数据集与7个真实基准，使用24个涵盖卷积和Transformer架构的预训练模型进行评估。评估指标包括生物识别验证指标、相似度得分分布、跨模型排名一致性以及各数据集的基础分布特性。

Result: 合成数据集的基准保真度差异较大，但MorphFace和Vec2Face两个最强候选能够复现真实基准的相对行为，其一致性水平落在真实基准之间的自然差异范围内。

Conclusion: 精心构建的合成数据集能够支持可靠的人脸识别比较评估，推动该领域向完全合成、隐私保护的训练和基准测试流程迈进。
...
TL;DR: 本文发布了两个番茄植物视觉数据集：BUTom21（静态图像+人工标注）和BUTom-ST21（视频数据+AI半自动伪标签），均提供像素级的果实成熟度标签，旨在为田间表型研究提供真实场景数据支持。


<details>
  <summary>Details</summary>
Motivation: 为研究社区提供在商业化种植条件下获取的、具有挑战性的真实世界番茄植物图像数据，以推动视觉传感和植物表型分析领域的方法探索。

Method: 构建了两个数据集：BUTom21包含静态图像和人工标注；BUTom-ST21包含视频数据并通过AI方法生成半自动标注（伪标签），两者均提供像素级果实成熟度标签。

Result: 成功发布了两个数据集，其中时空数据集BUTom-ST21提供了单个果实计数和成熟度信息。

Conclusion: 这些真实场景数据集有望推动基于田间的植物表型研究，特别是在果实计数和成熟度估计方面。
...
TL;DR: VideoChat3是一个完全开源、高效、通用的视频中心多模态大语言模型，通过I3D-ViT和自适应帧分辨率实现高效时空表示，通过可扩展数据合成流水线构建多样化训练数据集，在仅4B参数下超越同等或更大规模的开源模型。


<details>
  <summary>Details</summary>
Motivation: 现有开源视频理解模型存在三个局限：难以泛化到多种视频类型（仅在特定领域有效）、高计算需求限制效率和可扩展性、大部分模型仅部分开源（训练代码、策略或数据集不可用）阻碍可复现性和社区发展。

Method: 提出VideoChat3，包含两个互补设计：1) 效率方面：引入膨胀3D视觉Transformer(I3D-ViT)和自适应帧分辨率用于流式视频感知，实现高效时空表示并降低视频输入处理成本；2) 效果方面：开发可扩展的视频数据合成流水线，构建三个多样化高质量训练数据集(VideoChat3-Academic2M, VideoChat3-LV116K, VideoChat3-OL617K)，覆盖通用、长视频和流式视频场景。

Result: 在通用、长视频和流式视频基准测试中，VideoChat3仅用4B参数，在更高效率下超越了同等或更大参数量的现有开源模型。

Conclusion: VideoChat3在广泛泛化能力和计算效率之间实现了罕见的平衡，为视频领域多模态大模型的开源发展提供了完整解决方案。
...
TL;DR: 本文提出FreqLF，一种基于EPI引导的傅里叶-局部潜场学习框架，通过全局傅里叶低频更新和局部卷积在潜空间中联合建模光场视差，无需显式构建代价体，在HCI 4D光场基准上接近强监督基线。


<details>
  <summary>Details</summary>
Motivation: 现有光场视差估计方法依赖局部窗口、离散视差假设、大容量代价体或注意力聚合，难以同时满足光滑区域全局一致性与遮挡边界/薄结构处的局部精度。

Method: FreqLF从水平和垂直EPI堆叠中提取角度视差线索，结合中心视图外观特征投影到潜场；通过堆叠的混合傅里叶-局部层更新：傅里叶低模态更新实现全局特征交互，局部卷积保持空间细节；最后用坐标条件高斯混合解码器预测视差。

Result: 在HCI 4D光场基准上，FreqLF在基模型中避免显式代价体构建的同时接近强监督基线精度；消融实验证实傅里叶分支与局部分支互补，缩放实验证明跨空间分辨率的实用性。

Conclusion: 傅里叶-局部潜场学习是光场视差估计的一种有竞争力的替代方案，在全局一致性和局部精度之间取得了有效平衡。
...
TL;DR: 提出了一种基于推理时注意力动态调节的文本到图像模型安全方法，通过内省式注意力调控在不牺牲语义对齐和感知质量的前提下，有效阻止不安全内容生成，相比概念擦除机制具有更优越的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于流的文本到图像模型虽然生成能力出色，但容易生成不安全内容；已有的概念擦除、提示过滤、分类器门控等安全措施容易被参数高效微调等简单技术绕过，缺乏内在鲁棒性。

Method: 通过推理阶段的注意力动态内省分析，重新平衡图像合成过程中的注意力激活分布，将生成引导远离不安全概念，同时保持语义对齐。

Result: 在标准安全基准和对抗性安全基准上，该方法在取得优异安全得分的同时，保持甚至提升了语义对齐度和感知质量。

Conclusion: 注意力空间调控相比现有的概念擦除机制，为扩散Transformer图像生成的安全性问题提供了更有前景的解决路径。
...
TL;DR: DINE是一个最大后验框架，通过学习的位移向量场统计先验增强非刚性点云配准，在软组织形变场景下相比基线方法降低了27-79%的Chamfer距离，并显著提升了鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的非刚性点云配准方法主要依赖Chamfer距离等局部目标函数，只鼓励点对点邻近性，无法约束预测形变场的全局合理性，在大形变、噪声和外点场景下对应估计困难。

Method: 提出DINE框架，采用两阶段策略：第一阶段用Chamfer距离训练配准骨干网络（Robust-DefReg和DefTransNet）；利用预测的形变场估计先验（全场PCA高斯先验或逐向量归一化流先验）；第二阶段用距离目标加负对数先验目标联合优化精炼模型。

Result: 在DeformedTissue数据集上，DINE-PCA相对于Stage-1骨干将Chamfer距离降低约27-69%，对外点鲁棒性提升66%，对高斯噪声鲁棒性提升83%；在SynBench上，小形变改善有限，中等到严重形变改善约59-79%。

Conclusion: 全局形变合理性是可靠软组织点云配准的重要约束条件，将学习到的统计先验融入配准框架能显著提升准确性和鲁棒性。
...
TL;DR: UPrompt提出一种U型多粒度提示学习框架，通过粗到细的级联增强和细到粗的层次监督，解决视觉-语言模型中全局提示缺乏细粒度语义、局部提示忽略上下文关联的粒度困境，在17个基准上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型的提示学习面临粒度困境：全局提示缺乏细粒度语义感知，局部提示忽略上下文关联，限制了跨任务泛化能力，尤其在密集预测任务中这一问题更为突出。

Method: 受U-Net启发，UPrompt构建了一个U型多粒度提示学习框架，在视觉和文本模态中并行构建多粒度表示，包含粗到细的级联增强（全局上下文传播细化局部细节）和细到粗的层次监督（确保跨尺度语义一致性）。

Result: 在17个基准上验证有效性。在MSCOCO上，UPrompt分别超越MAMET和VPKE 4.1和7.3 rSum；在base-to-novel泛化上超越CoCoA-Mix 5.09%；粗粒度版本开销极小，中粒度以1/3成本达到PSRC性能。

Conclusion: UPrompt通过多粒度提示学习有效解决了粒度困境，在保持低开销的同时实现了跨任务泛化性能的提升。
...
TL;DR: 提出Stitch-Inferencer，一种通过在线全景拼接将隐式特征记忆替换为显式图像空间记忆的实时推理框架，无需重新训练即可提升内镜视频理解性能。


<details>
  <summary>Details</summary>
Motivation: 内镜手术视频中视野有限且器械频繁遮挡，现有视频模型依赖隐式特征记忆需要任务特定训练、大量标注数据和高计算成本，缺乏高效利用时空上下文的实用方案。

Method: 提出Stitch-Inferencer框架：在线将有效观测像素拼接至全景画布（显式图像空间记忆），在紧凑ROI上运行下游模型，再将预测重投影回当前帧，实现无需重训练的长程上下文利用。

Result: 在解剖分割和点/框追踪任务上，多种基线模型均取得一致提升；拼接模块运行速度超过60 FPS，保持实时吞吐量。

Conclusion: Stitch-Inferencer是一种实用的推理时解决方案，通过显式全景记忆高效增强内镜感知能力，适用于计算资源受限的术中环境。
...
TL;DR: 本文提出SimVLA，一个更简单但更高效的可迁移对抗攻击方法，揭示了当前VLP模型攻击中因不当跨模态交互和过多操作导致的三个被忽视问题，实验表明SimVLA在四个数据集上显著优于SOTA，同时大幅降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有针对VLP模型的可迁移攻击方法普遍采用复杂的损失函数或多阶段文本/图像攻击流程，但本文发现这些复杂设计反而存在因不当跨模态交互和过多操作导致的问题，因此希望提出一个更简单但更有效的攻击流水线。

Method: 提出SimVLA（Simple Vision-Language Attack）流水线，通过识别并解决三个被忽视的跨模态交互不当和操作过多问题，采用更简洁的设计实现攻击的可迁移性和效率提升。

Result: 在Flickr30k图文检索任务上，SimVLA在R@1迁移性上超越SOTA基线8.01%-14.71%，仅消耗约35.73%的时间和46.26%的最大显存；在四个数据集和三个下游任务上均验证了优越性。

Conclusion: 利用领域知识（如跨模态词识别）设计简单攻击方法比盲目追求复杂操作（如复杂损失函数和冗余多阶段设计）更有效，SimVLA可作为未来研究的简单有效基线。
...
TL;DR: 本文提出D2DF框架，通过Prior-Privileged Consistency Distillation（PPCD）将多步扩散模型的精炼能力蒸馏到单步生成模型中，并引入Self-Guided Fast Planting（SGFP）模块实现无草稿依赖的单步视频对象去除，在保证质量的同时将处理时间缩短至约1秒。


<details>
  <summary>Details</summary>
Motivation: 现有视频对象去除方法存在两难：传统方法（光流/注意力）易引入伪影效果不自然，而扩散方法虽质量高但需多步去噪效率低下。作者旨在兼顾高保真质量与单步高效率。

Method: (1) 训练一个多步教师模型将低质量草稿精炼为高保真视频；(2) 通过Prior-Privileged Consistency Distillation (PPCD) 将该能力蒸馏到单步学生模型中，学生模型以草稿为条件进行一步去除；(3) 引入Self-Guided Fast Planting (SGFP) 模块（基于Temporal Masked Transformer），在潜空间自主生成场景一致的伪草稿，实现无草稿依赖的完全单步模型。

Result: 在多个指标上均达到最先进水平，超越传统方法和多步生成方法。单视频去噪过程仅需约1秒。

Conclusion: D2DF框架成功弥合了传统方法与扩散方法之间的差距，在保持高视觉质量的同时实现了极致的推理效率，为视频对象去除提供了实用的解决方案。
...
TL;DR: JADE-GS通过像素自适应路由门融合物理先验与学习先验，并采用双向循环耦合2D修复器与3D高斯泼溅学生模型，实现高速运动模糊场景下的事件相机高质量3D重建，在合成与真实基准上取得最佳感知质量，训练仅需约1小时/5GB显存。


<details>
  <summary>Details</summary>
Motivation: 高速运动期间相机模糊会破坏曝光内运动信息，导致3D模型难以恢复清晰场景；事件相机虽能以微秒分辨率捕获运动信号，但现有方法面临两大障碍：(1)物理先验（事件积分）保边缘但累积漂移，学习先验（神经网络）恢复纹理但扭曲边界，二者互补却难以融合；(2)现有流水线仅单向运行，原始事件噪声或固定2D伪标签偏差会直接传递到几何重建中。

Method: 提出JADE-GS，核心包含：(1)像素自适应路由门（pixel-adaptive routing gate），动态融合物理事件积分先验与学习网络先验；(2)双向循环框架，将2D修复器与3D高斯泼溅（3DGS）学生模型耦合，通过解耦的多视图一致渲染和基于物理的重模糊约束来正则化修复器，将固定的预处理转化为几何感知预测器。

Result: 在合成与真实基准上，JADE-GS取得最佳感知质量，LPIPS和CLIP-IQA两项指标均领先，PSNR和SSIM也具有竞争力；训练仅需约1小时，显存消耗低于5GB，在单张消费级GPU上即可实现实时渲染。

Conclusion: JADE-GS通过双向耦合与自适应先验融合，有效解决了事件相机高速运动场景下的3D重建难题，证明了将2D修复器与3D几何模型联合优化的有效性。
...
TL;DR: Wan-Streamer v0.3 提出“视频=世界+事件流”的统一视角，将原生流式交互模型重构为在持久世界上下文中实时预测事件变化的通用预训练框架，并实例化于实时全双工音视频交互任务，保持v0.2的延迟性能指标。


<details>
  <summary>Details</summary>
Motivation: 现有视频交互模型缺乏统一的理论框架来整合环境上下文与动态变化，导致模型难以在持续流式场景中同时保持对稳定场景的理解和对实时事件的响应。

Method: 将视频建模为“世界”（持久上下文）与“事件流”（随时间变化的一切）的组合，设计通用预训练任务：给定世界和输入，实时预测世界如何运动、变化和响应。将该能力特化到实时全双工音视频交互，采用视觉-语言-动作（VLA）式的多模态理解过程，将多模态用户输入映射为语言形式的语音和行为动作。

Result: 保持v0.2的运行指标：640x368@25FPS视频、160ms流式单元、约200ms模型侧响应延迟、550ms总交互延迟（350ms双向网络预算下）。

Conclusion: “世界+事件流”的统一视角为视频流式交互提供了通用且可扩展的理论基础，能够在保持低延迟的同时支持广泛的实时下游任务。
...
TL;DR: 本文提出了一种基于SAM生成伪标注和条件扩散模型的弱监督RGB-D显著性检测方法，在7个数据集上优于同类弱监督方法，性能接近全监督方法。


<details>
  <summary>Details</summary>
Motivation: 弱监督RGB-D显著性检测中，涂鸦标注缺乏物体的结构与细节信息，导致生成的显著性图不准确。现有方法受限于稀疏的监督信号，难以获得高质量结果。

Method: 提出双模块框架：(1) SAM-PAG：利用SAM通过双分支结构和分割掩码一致性将稀疏涂鸦扩展为密集像素级伪标注；(2) S²Diff：基于条件扩散模型，通过跨模态条件生成模块（频率融合+隐式-显式状态空间交互）提取全局条件特征，并通过上下文注入模块去噪、增强目标信息，迭代优化噪声显著性图。

Result: 在7个公开数据集上，该方法优于现有涂鸦监督方法，性能与全监督方法相当。

Conclusion: SAM-PAG与S²Diff的协同工作有效解决了稀疏监督问题，验证了弱监督RGB-D SOD中扩散模型与SAM结合的巨大潜力。
...
TL;DR: 本文提出一种参数高效的轻量级框架，利用冻结的DINOv2-Small模型与三个模态特定的可学习提示令牌（仅1.19M可训练参数），通过共享交叉注意力层实现内在的空间可解释性，并引入MoCA自适应焦点损失函数处理认知边界模糊性，在MCI检测任务上以0.641的F1和0.795的AUC超越计算量更大的ResViT基线。


<details>
  <summary>Details</summary>
Motivation: 轻度认知障碍（MCI）是阿尔茨海默病的关键前驱阶段，但其自动化检测面临数据稀缺、类别不平衡及临床边界诊断模糊三大根本性挑战。现有方法依赖计算昂贵的全微调混合架构，且将空间可解释性视为事后近似而非内在模型属性。

Method: 1) 采用冻结的DINOv2-Small模型，通过三个模态特定的可学习提示令牌进行参数高效适配（仅1.19M可训练参数）；2) 每个令牌作为共享交叉注意力层中源图像块令牌的查询，直接通过注意力图实现内在的空间可解释性；3) 任务条件化嵌入通过注意力模块融合以量化每受试者的模态级重要性；4) 引入MoCA自适应焦点损失函数，将连续认知评分整合到训练目标、损失调制和自适应样本加权中，严格泛化标准软标签方法。

Result: 在分层五折交叉验证下，MCI类别F1达0.641，AUC达0.795，MCI类别F1比计算量更大的ResViT基线高出0.110。

Conclusion: 所提参数高效框架在数据稀缺和类别不平衡条件下显著优于计算密集型基线方法，并通过内在注意力机制提供了可靠的空间可解释性，为神经心理学绘画测试的自动化MCI检测提供了一种高效且可解释的解决方案。
...
TL;DR: ROADGS-T提出了一种基于自适应网格高斯表示的鲁棒高效大规模道路表面建图框架，通过2D高斯面元在网格上的分布、道路结构感知的自适应网格策略以及轨迹一致性引导的位姿鲁棒优化，显著提升了道路重建质量和效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于网格的道路表面重建方法在大规模驾驶场景中存在重建质量有限和优化成本高的问题，需要一种更高效且鲁棒的道路表面表示方法。

Method: 1) 在网格上放置2D高斯面元，显式存储颜色、语义和几何信息；2) 道路结构感知的自适应网格策略，在几何或语义复杂区域（如车道标线、道路边界）分配更密集的高斯面元；3) 轨迹一致性引导的位姿鲁棒优化策略，从多个邻近位姿估计局部表面先验并自适应加权位姿引导的高度正则化。

Result: 相比传统网格表示和3D高斯基元，所提网格高斯表示更匹配道路薄表面特性，显著减少冗余基元和重叠，同时提升了重建质量。

Conclusion: ROADGS-T在大规模道路表面建图任务中实现了鲁棒且高效的重建，兼顾了表示效率和结构保真度。
...
TL;DR: 本文提出ViPS框架，通过融合多个基础视觉模型提供的互补空间先验知识，并设计高效先验代理和动态先验融合机制，显著提升了多模态大模型在空间理解任务上的性能，在多个空间推理和3D理解基准上达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在空间理解中通常仅依赖单一预训练基础模型提取的先验知识，但不同基础模型提供的空间先验是互补的、适用于不同任务，现有方法未能充分利用多重先验的协同潜力。

Method: 提出ViPS框架，包含两个核心组件：1) Efficient Prior Proxy（高效先验代理），以最小推理开销生成多个基础先验；2) Dynamic Prior Fusion（动态先验融合），实现上下文感知的和谐先验融合与注入。

Result: 在多个复杂空间推理和3D空间理解基准上达到新的最优性能（SOTA），证实了所提框架能成功协调多种视觉先验。

Conclusion: 融合多模型互补空间先验并设计高效的动态融合机制，是提升MLLM空间理解能力的有效途径。
...
TL;DR: SUFLECA提出了一个弱监督框架，通过NOCS监督在67.4万张图像上学习几何感知特征，并结合几何一致性匹配算法，实现了零样本CAD-to-图像对齐，在ScanNet25k上以33.4%/42.3%的类别/实例精度超越了最强零样本基线10.3/12.2个百分点，甚至首次在该基准上超过了全监督方法。


<details>
  <summary>Details</summary>
Motivation: 现有零样本CAD-to-图像对齐方法依赖于外观驱动的匹配，在遮挡或仿真到真实域迁移场景下性能严重退化。

Method: 1）通过NOCS监督在67.4万张图像（12个真实与合成数据集）上缩放几何接地特征学习，从预训练视觉表示中学习紧凑的几何感知特征；2）提出几何一致性匹配算法，建立可靠的一对一CAD-to-图像对应关系。

Result: 在ScanNet25k上达到33.4%类别精度和42.3%实例精度，超越最强零样本基线10.3/12.2个百分点，计算开销更小，且首次在该基准上超越全监督方法。

Conclusion: SUFLECA通过大规模几何特征学习和一致性匹配，有效解决了外观驱动方法的局限性，实现了无需迭代优化的亚秒级高精度零样本CAD对齐。
...
TL;DR: 本文提出了两种报纸结构理解方法：基于YOLO+LayoutReader+自定义算法的模块化流水线，以及端到端Transformer架构Tiramisu，并发布了用于评估历史报纸层次信息检索的新数据集Finlam La Liberté。


<details>
  <summary>Details</summary>
Motivation: 报纸图像包含复杂的嵌套层次结构和密集异构布局，现有方法难以有效理解其结构。

Method: 1) 模块化自底向上流水线：YOLO进行布局检测，LayoutReader预测阅读顺序，自定义算法进行文章分割。2) Tiramisu：基于迭代分层Transformer的端到端架构，通过高度并行的注意力机制同步进行节/文章分离、块定位、语义分类和阅读顺序预测。

Result: 两种方法在重建复杂报纸层次结构上均有效，比较分析展示了各自在大规模文档数字化中的优势。

Conclusion: 模块化流水线灵活可解释，Tiramisu端到端模型并行高效，两者互补可用于报纸结构化理解。
...
TL;DR: 本文研究了人脸识别模型中成员与非成员嵌入向量在单位超球面上的几何聚类差异，发现训练身份数量是影响成员/非成员可分性的最主要因素，并提出了融合多种几何统计量的学习方法以增强成员推断能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于角度间隔损失训练的人脸识别模型，其嵌入向量在超球面上会形成身份聚类。但这些损失仅作用于训练集中的身份，因此非成员身份可能呈现出不同的几何聚类特性。本文旨在量化这种差异的大小及其受训练时各类因素控制的程度。

Method: 采用因子设计，在IResNet骨干网络大小、损失函数类型、训练时长和训练身份数量四个维度上构建了180个人脸识别模型。对每个模型计算四种基于聚类几何的统计量，并在九个基准数据集上评估性能。

Result: 训练身份数量对成员/非成员可分性的影响最大，骨干网络和损失函数贡献很小；在同域保留参考集上，几何成员信号随训练身份数量增加而单调递减；跨域非成员基准（姿态、年龄、质量、种族）会夸大成员信号；融合四种统计量的学习分类器能揭示超出单一最优统计量的额外成员信息。

Conclusion: 训练身份数量是影响成员推断中几何可分性的主导因素；跨域设置会引入虚假的成员信号；多统计量融合可提升成员推断能力。
...
TL;DR: AlphaWiSE通过后验权重空间插值方法，为每个参数张量拟合一个共享标量插值系数，在两个冻结的源检查点之间合成一个插值检查点，在不增加推理时间的情况下改善多模态持续学习中的跨模态对齐。


<details>
  <summary>Details</summary>
Motivation: 多模态模型在持续适应新数据时，早期阶段获得的跨模态对齐会被破坏。传统持续学习方法返回单一检查点，迫使所有检索方向在稳定性和可塑性之间做出相同的权衡，无法针对不同检索方向进行差异化优化。

Method: 提出AlphaWiSE，一种后验权重空间插值方法。该方法对两个冻结的源检查点，为每个由检查点键标识的对齐参数张量拟合一个共享标量插值系数，系数在小样本记忆上拟合，用于物化一个插值检查点，保持与原检查点相同的架构和参数量。

Result: 在音频-图像-文本检索任务上，AlphaWiSE在多个检索方向和评估指标上一致优于强持续学习基线方法。

Conclusion: AlphaWiSE通过参数级别的差异化插值，有效缓解了多模态持续学习中稳定性与可塑性的冲突，无需额外推理成本即可提升跨模态检索性能。
...
TL;DR: QuReC是一个统一的全能图像恢复框架，通过退化引导的查询重建模块（DQRM）和局部-全局响应校准模块（LGRCM），实现对空间异质或混合退化的自适应与高质量恢复。


<details>
  <summary>Details</summary>
Motivation: 现有全能图像恢复方法依赖图像级提示或共享引导，无法有效处理空间异质或混合退化，且空间自适应引导不足以确保局部和全局信息的可靠聚合。

Method: 提出QuReC框架，包含：(1) DQRM，通过将每个空间查询与退化原型空间匹配，重建查询特定的退化感知表征；(2) 弱监督原型匹配学习策略，提升优化稳定性和退化语义一致性；(3) LGRCM，进行局部-全局双分支聚合并通过可学习先验校准响应，改善局部细节与全局上下文的协调。

Result: 在多个全能图像恢复基准上取得了优越的性能。

Conclusion: QuReC通过细粒度空间自适应引导与局部-全局响应校准，有效解决了空间异质和混合退化问题，成为统一图像恢复的有效框架。
...
TL;DR: DAPGNet是一种动态自适应物理引导图扩散网络，通过注入光谱物理先验（连续波段的结构化约束）来改进高光谱图像分类。该方法首先提取节点级多尺度物理先验表示，然后构建物理先验感知的稀疏图拓扑，并在图扩散过程中通过物理门控机制融合图聚合特征与物理先验特征。在四个基准数据集上，DAPGNet在OA、AA和Kappa指标上均优于CNN、Transformer、Mamba和图基线方法，AA提升3.64–7.31个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的高光谱分类器通常依赖空间邻近性、超像素连通性或学习到的特征亲和性构建图拓扑，但忽视了连续波段所携带的光谱物理先验对拓扑估计和消息传播的约束作用，导致物理先验未能有效指导关系级图学习。

Method: DAPGNet包含四个核心模块：(1) 将连续光谱响应编码为节点级多尺度物理先验表示；(2) 两阶段图构造器结合光谱-空间亲和性、物理先验一致性和空间距离，构建物理先验感知的稀疏拓扑；(3) 图扩散中将学习到的边权重转化为加性注意力偏置，并通过物理门控在节点和特征维度上对图聚合特征与物理先验特征进行插值融合；(4) 跨尺度融合整合不同扩散深度的节点状态，优化目标包括主分类损失、辅助监督和二阶光谱平滑正则化。

Result: 在Indian Pines、WHU-Hi-LongKou、Houston2013和Houston2018四个数据集上，DAPGNet的OA、AA和Kappa均优于CNN、Transformer、Mamba及图基线方法，AA比最强竞争方法提升3.64–7.31个百分点。消融和敏感性分析验证了物理先验提取、先验感知拓扑、物理门控传播和光谱平滑正则化的互补效果。

Conclusion: 注入结构约束的光谱物理先验能够有效提升图神经网络在高光谱图像分类中的关系建模能力，物理先验与图扩散机制的深度融合是提升分类性能的关键。
...
TL;DR: 针对微型DOE投影仪FPP系统中非线性投影引起的相位伪影问题，提出了一种基于射线模型的相位误差校正框架，通过单方向双曲条纹估计投影仪针孔、单姿态标定构建校正模型，显著提升了重建精度。


<details>
  <summary>Details</summary>
Motivation: 微型DOE投影仪FPP系统存在严重的相位伪影，源于非线性投影特性和有限图案可控性，现有图像域处理方法未能充分利用投影仪几何结构。

Method: 1) 提出基于投影射线的相位误差校正框架，沿从投影仪针孔发出的射线建模相位伪影，融入投影仪几何；2) 提出基于单方向双曲条纹的投影仪针孔估计方法，无需立体标定即可恢复投影仪几何；3) 采用数据高效策略，仅需单姿态标定即可构建精化模型。

Result: 在微型DOE投影仪FPP系统上，非线性投影条件下重建精度显著提升，验证了方法的鲁棒性和物理一致性。

Conclusion: 该射线基物理建模方法有效解决了微型DOE投影仪FPP系统中的相位伪影问题，具有高鲁棒性和数据高效性。
...
TL;DR: MAGiSt3R是一个多智能体3D重建框架，利用3R系列的馈送模型和MAGMA合并模型，以近10 FPS的速度从单目RGB视频中完成重建和相机跟踪，并通过姿态图优化减少相机漂移，在合成和真实数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 实现从单目RGB视频中进行快速、准确的3D重建和相机跟踪，解决现有方法在速度和精度上的不足。

Method: 采用3R系列的馈送模型处理RGB视频并回归局部点图，通过MAGMA模型在智能体内部和智能体之间合并局部点图得到全局点图，并利用姿态图优化减轻馈送流水线中的累积相机漂移。

Result: 在合成和真实数据集上，MAGiSt3R的重建和相机跟踪精度均优于现有最先进方法。

Conclusion: MAGiSt3R通过多智能体协作和姿态图优化，实现了高效且精准的单目视频3D重建与相机跟踪。
...
TL;DR: 本文提出系统错位检测任务，用于发现多模态大语言模型在图像描述中重复出现的特定错误。作者提出Symbal方法，利用双阶段结构与现成基础模型自动检测并自然语言总结此类错误；同时构建SymbalBench基准（含170万图文对、420个数据集及标注），实验表明Symbal以63.8%的数据集识别率远超基线近4倍，并在真实场景中验证了其对多模型生成描述和现有数据集审计的有效性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型生成的图像描述常出现与特定视觉特征相关联的系统性重复错误（即系统错位），现有方法缺乏自动化检测此类错误的手段，且缺乏相应基准评估体系。

Method: 提出Symbal，采用双阶段结构：第一阶段利用现成基础模型检测系统错位，第二阶段用自然语言总结检测结果；构建SymbalBench基准，覆盖自然图像和医学图像两个领域，含170万图文对、420个数据集及人工标注的系统错位信息。

Result: Symbal在SymbalBench上正确识别63.8%数据集的系统错位，相比最优基线提升近4倍；在真实场景下对四种MLLM生成描述及现成图像描述数据集均有效。

Conclusion: 本文提出的新任务、方法和基准能够有效帮助用户审计MLLM生成的描述并识别关键错误，且无需访问底层MLLM模型。
...
TL;DR: 本文提出SSRL框架，通过结构-语义互惠学习将开放循环的跨模态关联转变为自校正的闭环系统，有效解决无监督可见光-红外行人重识别中全局表示模糊和伪标签噪声传播问题。


<details>
  <summary>Details</summary>
Motivation: 现有渐进式关联范式依赖模糊的全局表示，且伪标签噪声以开环方式 unchecked 传播，导致跨模态识别性能受限。

Method: 提出结构-语义互惠学习（SSRL）框架，包含两个核心模块：1）细粒度结构解耦（FSD），提取判别性身体部位基元作为空间锚点；2）闭环语义校准（CSC），在每个epoch重建共享语义原型并反馈回训练循环，过滤伪标签噪声。

Result: 在SYSU-MM01和RegDB数据集上达到SOTA性能，且在RegDB上超越部分有监督方法。

Conclusion: 结构学习与语义学习的互惠交互能够实现鲁棒的跨模态表示，将开环关联转变为自校正闭环系统是解决USVI-ReID的有效途径。
...
TL;DR: 本研究通过空间和时间两个维度的分析，比较了自闭症成人、专业艺术家和神经典型观察者在自由观看绘画时的眼动模式。研究发现：艺术家与神经典型者几乎无法区分，而自闭症群体的注视模式独特——空间上探索广泛但与艺术家不同，时间上表现为更短的注视时长、更少停留以及最不一致的扫描路径。自闭症者并不特别接近艺术家，反而略靠近神经典型者。这表明自闭症观看艺术是一种独特的群体特异性注意模式，需要群体条件化的审美注意模型。


<details>
  <summary>Details</summary>
Motivation: 以往关于自闭症的眼动研究多使用社交场景而非艺术作品，且只关注注视位置而忽略了时序信息。本研究旨在填补这一空白，探究不同人群（自闭症成人、专业艺术家、神经典型者）在自由观看艺术时的空间和时间注意模式差异。

Method: 三组被试（ASD、艺术家、神经典型者）各观看30幅绘画15秒。空间维度：用六种显著性指标（AUC-Judd、NSS、CC、SIM、KL、信息增益）比较一组注视密度图预测另一组注视的能力；时间维度：用MultiMatch、ScanMatch、Foveal-IoU（FDISS）和动态时间规整（DTW）比较个体扫描路径。所有眼动数据使用离散阈值算法统一提取注视点。

Result: (i)艺术家与神经典型者在空间（密度图相关CC=0.96）和时间上几乎不可区分，而ASD与两者均不同。(ii)ASD注意力分离：空间探索范围与艺术家相当，但时间特征独特（注视更短、停留更少、扫描路径最不一致）。(iii)ASD在任何指标上都不特别像艺术家，反而略接近神经典型者。

Conclusion: 自闭症观看艺术是一种独特的群体特异性注意配置，同时体现在空间和时间维度上，需要发展群体条件化的审美注意模型来更好地理解和适应这一差异。
...
TL;DR: 本文提出CRISP框架，利用“正区域排名稳定性”假设，通过概率排名稳定性推导鲁棒空间先验，无需目标域数据或测试时参数更新即可实现源域训练模型的域外分割，在多中心心脏MRI和肺部CT血管分割任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 医学影像中的分布偏移是临床AI翻译的核心瓶颈，现有域适应方法受限于预定义模拟偏移或伪监督，无法应对开放世界中无限且不可预测的真实分布偏移。

Method: 采用“正区域排名稳定性”假设，利用概率排名在分布偏移下的稳定性推导鲁棒空间先验；通过潜在特征扰动获取扰动不变的高精度（HP）核心区域和至少一次扰动下保持前景的高召回率（HR）支持区域；设计迭代训练框架逐步将HP和HR压缩至最终分割结果。CRISP是模型无关的即插即用框架，无需测试时参数更新或目标域数据。

Result: 在多中心心脏MRI和肺部CT血管分割任务上，CRISP显著优于SOTA方法，HD95分别降低0.14（7.0%）、1.90（13.1%）和8.39（38.9%）像素，涵盖多中心、人口统计和模态偏移场景。

Conclusion: CRISP通过排名稳定性假设有效解决了医学图像分割中开放世界分布偏移问题，提供了无需目标域数据的源域仅训练鲁棒分割方案。
...
TL;DR: 本文提出ARMOR++，一种利用视觉语言模型(Qwen2.5-VL)和大型语言模型(Qwen3)协同驱动的多智能体框架，通过五种互补扰动原语和熵正则化混合策略，显著提升黑盒对抗迁移攻击对深度伪造检测器的逃逸能力，在AADD-2025基准上取得超越现有基线的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测器在黑盒对抗迁移下可靠性下降，因为它们依赖脆弱的架构相关取证线索；现有迁移攻击缺乏语义感知能力，且在严格无查询约束下难以保持有效性，尤其是从卷积代理模型向Transformer目标模型迁移时性能不佳。

Method: ARMOR++框架包含：(1) Qwen2.5-VL视觉语言模型提供空间语义先验；(2) Qwen3大语言模型负责原语选择、自适应超参数重参数化和熵正则化扰动混合；(3) 五种互补扰动原语：密集优化、显著性方法、空间变换、频域扰动和块结构修改，协同攻击异构归纳偏置。

Result: 在AADD-2025基准上，ARMOR++在低质量和高质量图像场景下均显著优于现有智能体与非智能体基线；盲目标攻击成功率(ASR)相比最先进的智能体基线有大幅提升；在鲁棒防御配置下仍保持性能优势。

Conclusion: 当前深度伪造检测器部署存在显著的剩余可靠性差距，智能体化编排在识别潜在脆弱性方面具有有效性。
...
TL;DR: 本文针对视觉语言模型（VLM）在图像地理定位中存在的 landmark bias 问题，设计了量化指标 BI 和 BH 并构建了基准 LandmarkBias-3K，提出了基于证据驱动的推理框架 HoloGeo，配合高质量数据集 BF-30k 和多维奖励机制，有效缓解了 landmark bias，在多个基准上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 在地理定位任务中受 landmark bias 影响，容易忽略地理线索或形成虚假关联，导致定位不准确。为此作者希望系统性研究并量化该偏差，并提出缓解方法。

Method: (1) 设计 Bias Intensity (BI) 和 Bias Harmfulness (BH) 两个量化指标；(2) 构建 LandmarkBias-3K 基准；(3) 提出 HoloGeo 证据驱动推理框架，配合 BF-30k 高质量数据集（含结构化多证据无偏推理链），通过多维奖励机制鼓励模型均衡关注多类视觉线索，实现证据驱动的联合推理。

Result: HoloGeo 在 IM2GPS3K 和 YFCC4k 上保持优秀性能，同时在 LandmarkBias-3K 上显著优于现有开源 VLM，验证了其在鲁棒地理空间推理上的有效性。

Conclusion: HoloGeo 能有效缓解 landmark bias，提升地理定位的可靠性和鲁棒性。
...
TL;DR: 提出MCF-Net，一种运动引导的多视图融合框架，结合心肌运动线索与基础模型表征，通过极稀疏监督（单模板帧初始化点追踪）和运动条件融合机制，在心肌梗死节段定位任务上达到72.4% F1和84.9%准确率，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有心肌运动分析方法依赖手工特征或密集监督，标注需求大；基础模型虽提升了超声心动图分析性能，但多数方法仅操作单视图，且在尖段视图下节段定位存在视角依赖的模糊性问题，不可靠。

Method: 1) 使用预训练的Echo基础模型EchoPrime提取双视图视觉特征；2) 极稀疏监督运动建模：单张标注模板帧跨视频传播，初始化点追踪，避免密集标签；3) 运动推导的节段感知软掩码提供粗略空间先验，增强困难心肌节段特征；4) 运动条件融合机制跨视图融合运动与视觉信息，精细化预测。

Result: 节段级心肌梗死定位F1达72.4%，准确率84.9%，优于现有运动-only、视觉-only及融合基线方法。

Conclusion: MCF-Net通过运动引导的多视图融合与极稀疏监督，有效解决了单视图下节段定位不可靠的问题，实现了更优的心梗节段定位性能。
...
TL;DR: 针对多视角流式视频的实时新视角合成任务，本文提出通过解耦记忆更新与应用的频率（周期更新、逐帧应用），结合跨视图注意力、记忆损失和记忆缓存策略，在实时性要求下实现对动态场景的长时记忆与高质量重建。


<details>
  <summary>Details</summary>
Motivation: 在线新视角合成面临实时约束与长时记忆之间的根本矛盾：标准TTT模型每帧进行梯度更新计算成本过高，无法实时运行且长上下文中不稳定。

Method: 1) 将记忆更新与应用频率解耦，周期性更新记忆但逐帧应用；2) 跨视图注意力处理当前帧与历史记忆状态间的变形；3) 辅助记忆损失(强制场景内化)；4) 记忆缓存策略(正则化主动权重防止灾难性漂移)。

Result: 在动态人体运动场景上实现了实时且最先进的性能，同时支持分钟级别的在线记忆。

Conclusion: 通过频率解耦和辅助正则化机制，有效平衡了实时约束与长时记忆需求，验证了该方法在动态场景实时新视角合成中的有效性。
...
TL;DR: MeanFlowNFT将强化学习引入MeanFlow快速采样生成器，通过构建瞬时速度预测器并应用DiffusionNFT目标，在保持少步生成优势的同时实现了基于奖励的优化，在图像和视频生成任务上超越了现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: DiffusionNFT等RL方法成功优化了扩散模型，但尚未应用于MeanFlow生成器，因为前者优化瞬时速度而后者使用平均速度采样，两者存在本质差异。

Method: 基于MeanFlow恒等式连接平均速度与瞬时速度，构建诱导的瞬时速度预测器，将DiffusionNFT目标应用于该预测器，同时采样仍使用平均速度以保持少步生成优势。

Result: 在SD3.5-M上8个指标中6个超越先前最先进RL微调少步生成器；在Wan 2.1上4步MeanFlowNFT达到VBench 84.33，超越50步LongCat-Video RL的82.57。

Conclusion: MeanFlowNFT成功弥合了DiffusionNFT与MeanFlow之间的理论鸿沟，具备严格的策略改进保证，能以极少采样步数超越多步RL调优的扩散模型。
...
TL;DR: HDR是一种层次化去噪框架，将视频潜变量组织成树状结构，在因果视频生成中实现从粗到精的多步推理。相比流式自回归扩散模型，成功率从34.22提升至60.29（相对提升76.2%）；相比双向扩散，推理速度提升54.2倍，仅需2%训练数据即可保留82.9%性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频模型缺乏类人多步推理能力：流式自回归扩散模型高效但推理能力有限，双向扩散可全局修正但推理成本高。两者均难以在复杂推理任务中同时实现逻辑一致性与低延迟流式输出。

Method: 提出层次化去噪推理框架(HDR)，将视频潜变量组织成树结构层次。粗去噪层保留不确定假设用于全局规划，细层逐步精化具体视觉状态。采用稀疏层次注意力模式(SHAP)降低时序注意力成本。

Result: 在六项多步推理任务中：相比流式自回归扩散基线，成功率从34.22提升至60.29（+76.2%），平均进度从76.00提升至89.56。推理延迟0.70秒/潜变量，比双向扩散快54.2倍。2%训练数据保留82.9%性能（双向扩散仅52.0%）。真实机器人实验验证了物理交互与世界建模潜力。

Conclusion: HDR通过层次化潜变量结构有效平衡了推理质量与效率，为视频模型赋予类人多步推理能力，并在真实机器人场景中展现应用前景。
...
TL;DR: URVC是一个统一的实时神经视频编码框架，通过时间、空间和感知三个维度的自适应机制，解决了现有神经视频编码器在动态环境中适应性差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有神经视频编码器在部署到动态环境时表现出严重的刚性：无法适应不同视频内容、用户需求和质量偏好。具体存在三个局限性：1) 为满足实时性丢弃了显式运动估计和运动压缩，无法自适应时间预测；2) 空间比特分配策略粗粒度且固定，无法在测试时动态调整；3) 无法在不部署单独模型的情况下适应不同的质量偏好需求。

Method: 1) 速率感知自适应时间预测：通过多候选架构生成多样化的预测候选，并将候选选择直接耦合到率失真优化中；2) 基于分解的空间速率控制：通过特征分解和分离量化实现更细粒度的空间比特分配，支持测试时直接控制而无需重新训练；3) 感知切换方法：仅需学习一个辅助模块库配合帧生成器，即可在信号保真度和感知质量模式间切换。

Result: 提出的URVC框架在单一模型中同时实现了时间、空间和感知三个维度的自适应能力，克服了现有方法的三个主要局限性。

Conclusion: URVC将刚性的神经视频编码系统转化为统一的、具有时间、空间和感知自适应性的框架，使单个码流能够适应动态环境中的多样化需求。
